...
class SinkhornDROLogisticRGO(BaseLinearDRO):
    ...
    def _rgo_sampler_vectorized(self, x_original_sample_xi: torch.Tensor, y_original_sample: torch.Tensor,
        ...
        for _ in range(int(min(5, self.rgo_inner_steps * epoch/self.max_iter))):
            ...
            f_model_loss = self._get_model_loss_value_scalar_for_grad(x_pert, y_original_sample, current_model_state)
        ...
        f_model_loss_opt_star = self._get_model_loss_value_scalar_for_grad(x_opt_star, y_original_sample, current_model_state)
...
if __name__ == '__main__':
    ...
    sl_dro_model = SinkhornLinearDRO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=8e-2, batch_size=DRO_BATCH_SIZE)
    sl_dro_model.fit(train_features_np, train_labels_np)


    print("\n--- Training SinkhornDROLogisticRGO Model ---")
    rgo_dro_model = SinkhornDROLogisticRGO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=8e-2, batch_size=1)
    rgo_dro_model.fit(train_features_np, train_labels_np)
    
    print("\n--- Training SinkhornDROLogisticSVGD Model ---")
    svgd_dro_model = SinkhornDROLogisticSVGD(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=8e-2, batch_size=DRO_BATCH_SIZE)
