...
class SinkhornDROLogisticRGO(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, epsilon: float = 0.1,
                 ...
                 num_samples: int = 8, max_iter: int = 30, learning_rate: float = 0.01,
                 batch_size: int = 64, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
        ...
        self.num_samples, self.max_iter = num_samples, max_iter
    ...
    def _rgo_sampler_vectorized(self, x_original_batch: torch.Tensor, y_original_batch: torch.Tensor,
        ...
        """
        Performs fully vectorized RGO sampling using 3D tensors to ensure correctness.
        """
        batch_size = x_original_batch.size(0)
        
        # === Part 1: Vectorized Optimization to find x_opt_star (remains the same) ===
        ...
        for _ in range(inner_steps):
            ...
            per_sample_grads, = torch.autograd.grad(
                outputs=per_sample_losses, inputs=x_pert_batch, grad_outputs=torch.ones_like(per_sample_losses)
            )
        ...
        x_opt_star_batch = x_pert_batch # Shape: [B, D]

        # === Part 2: Vectorized Rejection Sampling using 3D Tensors ===
        ...
        # Calculate loss and norm for the optimal points, shape: [B]
        ...
        # --- Prepare tensors for 3D operations ---
        ...
        # Final perturbations will be stored here, shape [B, N, D]
        final_accepted_perturbations = torch.zeros((batch_size, num_samples_to_generate, self.input_dim), device=self.device)
        # Active flags for each of the N samples for each of the B batch items, shape [B, N]
        ...
        for _ in range(self.rgo_vectorized_max_trials):
            ...
            # Generate proposals for all B*N slots, shape [B, N, D]
            pert_proposals = torch.randn_like(final_accepted_perturbations) * std_rgo
            
            # Broadcasting [B, 1, D] + [B, N, D] -> [B, N, D]
            x_candidates = x_opt_star_3d + pert_proposals
            
            # For model evaluation, flatten the first two dimensions
            x_candidates_flat = x_candidates.view(-1, self.input_dim) # Shape: [B*N, D]
            y_repeated = y_original_batch.repeat_interleave(num_samples_to_generate, dim=0) # Shape: [B*N]
            
            f_model_loss_candidates_flat = self._get_model_loss_value_batched(x_candidates_flat, y_repeated, current_model_state)
            # Reshape loss back to 3D structure, shape [B, N]
            f_model_loss_candidates = f_model_loss_candidates_flat.view(batch_size, num_samples_to_generate)
            
            # Broadcasting [B, N, D] - [B, 1, D] -> [B, N, D]. Then sum over D. Shape: [B, N]
            ...
            diff_cand_opt_norm_sq = torch.sum(pert_proposals**2, dim=2) # Shape: [B, N]
            exponent_term3 = diff_cand_opt_norm_sq / (2 * var_rgo)
            
            # Broadcasting [B, N] - [B, 1] -> [B, N]. This is the crucial step.
            prob_exp_argument = torch.clamp(-f_L_xi_candidates + f_L_xi_opt_star_3d + exponent_term3, max=10)
            acceptance_probs = torch.exp(prob_exp_argument)
            
            rand_unif = torch.rand_like(acceptance_probs)
            
            # Identify newly accepted samples among those that are still active
            newly_accepted_mask = (rand_unif < acceptance_probs) & active_flags
            
            # Update final perturbations and flags
        ...
        # Add the accepted perturbations to their corresponding optimal points via broadcasting
        final_samples_3d = x_opt_star_3d + final_accepted_perturbations # Shape: [B, N, D]
        
        # Return the flattened result for the training step
        return final_samples_3d.view(-1, self.input_dim) # Shape: [B*N, D]
        ...
        for epoch in range(self.max_iter):
            ...
            for x_original_batch, y_original_batch in pbar:
                ...
                x_rgo_batch = self._rgo_sampler_vectorized(
                    x_original_batch, y_original_batch, self.model, self.num_samples, epoch
                )
...
if __name__ == '__main__':
    ...
    rgo_dro_model = SinkhornDROLogisticRGO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE)
    rgo_dro_model.fit(train_features_np, train_labels_np)

    print("\n--- Training SinkhornLinearDRO Model ---")
    sl_dro_model = SinkhornLinearDRO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE)
    sl_dro_model.fit(train_features_np, train_labels_np)

    # print("\n--- Training SinkhornDROLogisticSVGD Model ---")
    # svgd_dro_model = SinkhornLinearDRO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE)
