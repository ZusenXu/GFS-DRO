...
class SinkhornDROLogisticRGO(BaseLinearDRO):
    ...
    def _rgo_sampler_vectorized(self, x_original_sample_xi: torch.Tensor, y_original_sample: torch.Tensor,
        ...
        num_inner_steps = int(min(5, self.rgo_inner_steps * epoch / self.max_iter)) if self.max_iter > 0 else 0
...
if __name__ == '__main__':
    ...
    rgo_dro_model = SinkhornDROLogisticRGO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=8e-2, batch_size=DRO_BATCH_SIZE)
    rgo_dro_model.fit(train_features_np, train_labels_np)
    
    print("\n--- Training SinkhornDROLogisticSVGD Model ---")
    svgd_dro_model = SinkhornLinearDRO(INPUT_DIM, NUM_CLASSES, device=DEVICE, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=8e-2, batch_size=DRO_BATCH_SIZE)
