...
if __name__ == '__main__':
    ...
    if not os.path.exists(file_path):
        ...
        DRO_BATCH_SIZE = 128
        lam, eps, itr = 100, 0.01, 10 # Hyperparameters
        ...
        all_runs_training_losses = {"LD": [], "Base": [], "RGO": []}
        all_runs_eval_results = {"LD": [], "SAA": [], "Base": [], "RGO": []}
        ...
        for i in range(N_REPEATS):
            ...
            LD_dro_model = SinkhornDROLD(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=5e-3, batch_size=DRO_BATCH_SIZE, num_samples=8)
            ...
            base_dro_model = SinkhornBaseDRO(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=5e-3, batch_size=DRO_BATCH_SIZE, sample_level=5)
            base_loss_history = base_dro_model.fit(train_features_np, train_labels_np, checkpoint_dir=CHECKPOINT_DIR, run_id=i)
            all_runs_training_losses["Base"].append(base_loss_history)

            print("\n--- Training SinkhornDROLogisticRGO Model ---")
            rgo_dro_model = SinkhornDROLogisticRGO(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=5e-3, batch_size=DRO_BATCH_SIZE, num_samples=8)
