...
class SinkhornBaseDRO(BaseLinearDRO):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        for epoch in range(self.max_iter):
            ...
            # <<< 新增：在每个 epoch 结束后保存模型 >>>
            torch.save(self.model.state_dict(), f'checkpoints/SinkhornBaseDRO_epoch_{epoch+1}.pth')

        return loss_history


class SinkhornDROLogisticRGO(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, epsilon: float = 0.1, lambda_param: float = 1.0, rgo_inner_lr: float = 0.05, rgo_inner_steps: int = 20, num_samples: int = 10, max_iter: int = 30, learning_rate: float = 0.01, batch_size: int = 64, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> List[float]:
        ...
        for epoch in range(self.max_iter):
            ...
            # <<< 新增：在每个 epoch 结束后保存模型 >>>
            torch.save(self.model.state_dict(), f'checkpoints/SinkhornDROLogisticRGO_epoch_{epoch+1}.pth')
...
class SinkhornDROLD(BaseLinearDRO):
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> List[float]:
        ...
        for epoch in range(self.max_iter):
            ...
            # <<< 新增：在每个 epoch 结束后保存模型 >>>
            torch.save(self.model.state_dict(), f'checkpoints/SinkhornDROLD_epoch_{epoch+1}.pth')
...
def train_saa(model, train_loader, epochs=30, learning_rate=0.001):
    ...
    for epoch in range(epochs):
        ...
        # <<< 新增：在每个 epoch 结束后保存模型 >>>
        torch.save(model.state_dict(), f'checkpoints/SAA_epoch_{epoch+1}.pth')
