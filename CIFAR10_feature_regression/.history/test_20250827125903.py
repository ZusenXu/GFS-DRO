...
# --- ‼️ UPDATED PARAMETERS AS PER YOUR REQUEST ‼️ ---
INPUT_DIM = 250
TOTAL_CLASSES = 10
LAMBDA_PARAM = 10
EPSILON = 0.01
...
if __name__ == '__main__':
    ...
    # Note: Ensure the loaded features match the new INPUT_DIM
    # This code will slice the features if they are larger than INPUT_DIM
    ...
    # Lists to store filenames for GIF creation
    rgo_filenames = []
    ld_filenames = []
    ...
    for epoch in tqdm(range(1, MAX_EPOCHS + 1), desc="Processing Epochs"):
        for method_name, (ModelClass, file_prefix) in methods_to_visualize.items():
            ...
            fig.savefig(plot_filename, dpi=100) # Lower DPI for faster GIF creation
            plt.close(fig)
            
            # Add filename to the correct list
            if method_name == "RGO":
                rgo_filenames.append(plot_filename)
            else:
                ld_filenames.append(plot_filename)
    ...
    if imageio:
        ...
        for method_name, filenames in [("RGO", rgo_filenames), ("LD", ld_filenames)]:
            if filenames:
                gif_path = os.path.join(PLOT_DIR, f"{method_name}_perturbation_evolution.gif")
                with imageio.get_writer(gif_path, mode='I', duration=500, loop=0, format='gif') as writer: # duration is in ms
