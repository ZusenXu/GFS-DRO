...
class SinkhornDROLD(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, epsilon: float = 0.1, lambda_param: float = 1.0, num_samples: int = 4, max_iter: int = 30, learning_rate: float = 0.01, batch_size: int = 64, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
        self.epsilon, self.lambda_param = epsilon, lambda_param
        self.num_samples, self.max_iter, self.learning_rate, self.batch_size = num_samples, max_iter, learning_rate, batch_size
    ...
    def _LD_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        # Use a dummy loss just to get the gradient w.r.t inputs
...
if __name__ == '__main__':
    ...
    for epoch in tqdm(range(1, MAX_EPOCHS + 1), desc="Processing Epochs"):
        for method_name, (ModelClass, file_prefix) in methods_to_visualize.items():
            ...
            dro_instance.model.load_state_dict(torch.load(model_path, map_location=DEVICE,  weights_only=True))
            ...
            if method_name == "RGO":
                X_perturbed_torch = dro_instance._rgo_sampler_vectorized(X_sample_torch, y_sample_torch, dro_instance.model, 4, epoch)
            else: # LD
