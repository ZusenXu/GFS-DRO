...
def pgd_attack(model, features, labels, epsilon, alpha, num_iter):
    ...
    # Use the custom loss function that explicitly matches the paper's formula
...
# --- 4. Model and Custom Loss Definitions ---

# NEW: Custom loss function to explicitly match the paper's formula
class PaperNegativeLogLikelihoodLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        # This term corresponds to log(1^T e^(B^T x))
        log_sum_exp = torch.logsumexp(logits, dim=1)

        # This term corresponds to -y^T B^T x
        # We gather the logits for the true class indices
...
class BaseLinearDRO:
    ...
    def _compute_loss(self, predictions, targets, m, lambda_reg):
        ...
        criterion = PaperNegativeLogLikelihoodLoss(reduction='none')
...
class SinkhornDROLogisticRGO(BaseLinearDRO):
    ...
    def _get_model_loss_value_batched(self, x_features_batch: torch.Tensor, y_target_batch: torch.Tensor, model_instance: nn.Module) -> torch.Tensor:
        ...
        return PaperNegativeLogLikelihoodLoss(reduction='none')(predictions_logits_batch, y_target_batch)
