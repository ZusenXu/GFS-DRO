...
class SinkhornDROLD(BaseLinearDRO):
    ...
    def _LD_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        loss = nn.BCEWithLogitsLoss(reduction='sum')(predictions, y_orig)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(file_path):
        ...
        # rgo_dro_model = SinkhornDROLogisticRGO(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_iter=2, learning_rate=1e-3, batch_size=DRO_BATCH_SIZE, noise=NOISE_LEVEL)
        ...
        all_results = {}
        models_to_evaluate = { "SAA": saa_model_wrapper, "Base": base_dro_model, "LD": LD_dro_model }
