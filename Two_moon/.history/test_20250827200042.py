...
def plot_frame(model, true_boundary_model, X, y, X_perturbed, title, save_path, method):
    ...
    if X_perturbed is not None:
        ...
        sns.kdeplot(x=X_p_np[y_repeated==1, 0], y=X_p_np[y_repeated==1, 1], ax=ax, color='red', fill=True, alpha=0.4)
        sns.kdeplot(x=X_p_np[y_repeated==0, 0], y=X_p_np[y_repeated==0, 1], ax=ax, color='blue', fill=True, alpha=0.4)
    ...
    ax.contour(xx, yy, Z_model, levels=[0.5], colors=['green'], linestyles=['--'], linewidths=3)
    legend_elements = [Line2D([0], [0], color='green', linestyle='--', lw=3, label=f'{method} Boundary'), Patch(facecolor='red', alpha=0.4, label='Positive Worst-Case Dist.'), Patch(facecolor='blue', alpha=0.4, label='Negative Worst-Case Dist.')]
    ax.legend(handles=legend_elements, loc='best'); ax.set_title(title); ax.set_xlim(xx.min(), xx.max()); ax.set_ylim(yy.min(), yy.max())
...
def plot_loss_comparison(loss_histories: Dict[str, list], save_path):
    ...
    for name, history in loss_histories.items():
        epochs_logged = range(1, (len(history) * LOG_EPOCH_INTERVAL) + 2, LOG_EPOCH_INTERVAL)
...
def plot_robustness_loss_comparison(history: Dict[str, Dict[str, list]], save_dir: str):
    attack_types = ["Gaussian", "Laplace"]
    for attack in attack_types:
        plt.figure(figsize=(12, 8))
        
        for model_name, attack_history in history.items():
            losses = attack_history[attack]
            if not losses:
                continue
            epochs_logged = range(1, (len(losses) * ROBUSTNESS_LOG_INTERVAL) + 1, ROBUSTNESS_LOG_INTERVAL)
            plt.plot(epochs_logged, losses, label=model_name, marker='o', markersize=8)

        plt.xlabel("Epoch")
        plt.ylabel("Test Loss under Attack")
        plt.title(f"Model Robustness (Test Loss) vs. Training Epochs ({attack} Attack)")
        plt.legend()
        plt.grid(True)
        # plt.ylim(0, 1.05) # Removed ylim to auto-scale for loss
        plt.gca().xaxis.set_major_locator(plt.MaxNLocator(integer=True))
        
        save_path = os.path.join(save_dir, f"robustness_loss_comparison_{attack.lower()}.png")
        plt.savefig(save_path)
        plt.close()
        print(f"{attack} robustness loss plot saved to {save_path}")
...
def evaluate_loss(model, x_test, y_test, criterion):
    model.eval()
    with torch.no_grad():
        outputs = model(x_test)
...
if __name__ == '__main__':
    # --- Data ---
    X_train, y_train = create_dataset(n_samples=N_SAMPLES_TRAIN, noise=0, imbalance_ratio=0.8, random_state=SEED)
    X_test, y_test = create_dataset(n_samples=N_SAMPLES_TEST, noise=0.2, random_state=SEED+1)
    ...
    robustness_loss_history = {name: {"Gaussian": [], "Laplace": []} for name in model_names}
    ...
    for epoch in tqdm(range(MAX_EPOCHS), desc="SAA Training"):
        ...
        if (epoch + 1) % ROBUSTNESS_LOG_INTERVAL == 0:
            loss_gauss = evaluate_loss(saa_model, X_test_gauss, y_test_torch, eval_criterion)
            loss_laplace = evaluate_loss(saa_model, X_test_laplace, y_test_torch, eval_criterion)
            robustness_loss_history["SAA"]["Gaussian"].append(loss_gauss)
            robustness_loss_history["SAA"]["Laplace"].append(loss_laplace)
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if epoch % ROBUSTNESS_LOG_INTERVAL == 0:
                loss_gauss = evaluate_loss(model, X_test_gauss, y_test_torch, eval_criterion)
                loss_laplace = evaluate_loss(model, X_test_laplace, y_test_torch, eval_criterion)
                robustness_loss_history[method]["Gaussian"].append(loss_gauss)
                robustness_loss_history[method]["Laplace"].append(loss_laplace)
            ...
            if method in ["RGO", "LD"]:
                ...
                plot_frame(model, true_boundary_model, X_train, y_train, epoch_X_p, f"{method} Distribution (Epoch {epoch+1})", frame_path, method)
    ...
    plot_robustness_loss_comparison(robustness_loss_history, OUTPUT_DIR)
    
    # 4. Final Robustness Evaluation (Accuracy)
    print("\n--- Evaluating Final Model Robustness against Noise Attacks (Accuracy) ---")
    print("-" * 70)
    ...
    # 5. Final Robustness Evaluation (Test Loss)
    print("\n--- Evaluating Final Model Test Loss against Noise Attacks ---")
    print("-" * 70)
    print(f"Attack Strength (Epsilon): {ATTACK_EPSILON}")
    print("-" * 70)
    for name, model in trained_models.items():
        loss_clean = evaluate_loss(model, X_test_torch, y_test_torch, eval_criterion)
        loss_gauss = evaluate_loss(model, X_test_gauss, y_test_torch, eval_criterion)
        loss_laplace = evaluate_loss(model, X_test_laplace, y_test_torch, eval_criterion)
        print(f"| Model: {name:<12} | Clean Loss: {loss_clean:7.4f} | Gaussian Loss: {loss_gauss:7.4f} | Laplace Loss: {loss_laplace:7.4f} |")
    print("-" * 70)
