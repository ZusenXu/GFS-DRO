...
# Attack parameters
ATTACK_EPSILON = 0.2 # Strength of the noise attack
...
def create_dataset(n_samples=500, noise=0.15, imbalance_ratio=0.8, random_state=42):
...
def plot_frame(model, true_boundary_model, X, y, X_perturbed, title, save_path, method):
    ...
    legend_elements = [Line2D([0], [0], color='black', lw=3, label='True Boundary'), Line2D([0], [0], color='green', linestyle='--', lw=3, label=f'{method} Boundary'), Patch(facecolor='red', alpha=0.4, label='Positive Worst-Case Dist.'), Patch(facecolor='blue', alpha=0.4, label='Negative Worst-Case Dist.')]
...
def plot_loss_comparison(loss_histories: Dict[str, list], save_path):
    ...
    for name, history in loss_histories.items():
        plt.plot(history, label=name, marker='o', linestyle='--', markersize=4)
    plt.xlabel("Epoch")
...
if __name__ == '__main__':
    # --- Data ---
    X_train, y_train = create_dataset(n_samples=N_SAMPLES_TRAIN, noise=0.15, random_state=SEED)
    X_test, y_test = create_dataset(n_samples=N_SAMPLES_TEST, noise=0.15, random_state=SEED+1)
    ...
    loss_histories = {name: [] for name in ["SAA", "RGO", "LD", "SinkhornBase"]}
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["RGO", "LD", "SinkhornBase"]
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                elif method == "SinkhornBase":
                    levels = np.arange(SINKHORN_SAMPLE_LEVEL + 1)
                    numerators = 2.0**(-levels); denominator = 2.0 - 2.0**(-SINKHORN_SAMPLE_LEVEL)
            ...
            # --- Visualization (Skip for SinkhornBase) ---
            if method in ["RGO", "LD"]:
                model.eval()
        ...
        # --- Create GIF (Skip for SinkhornBase) ---
