...
def plot_robustness_comparison(history: Dict[str, Dict[str, list]], save_dir: str):
    """Plots the robustness evolution against different attacks."""
    ...
    for attack in attack_types:
        ...
        for model_name, attack_history in history.items():
            ...
            if not accuracies:
                continue
            epochs_logged = range(ROBUSTNESS_LOG_INTERVAL, (len(accuracies) * ROBUSTNESS_LOG_INTERVAL) + 1, ROBUSTNESS_LOG_INTERVAL)
            plt.plot(epochs_logged, accuracies, label=model_name, marker='o', linestyle='--', markersize=4)
...
if __name__ == '__main__':
    ...
    LOG_EPOCH_INTERVAL = 4
    ROBUSTNESS_LOG_INTERVAL = 10 # NEW: Log robustness every 10 epochs
    
    model_names = ["SAA", "RGO", "LD", "Dual"]
    loss_histories = {name: [] for name in model_names}
    # NEW: Dictionary to store robustness history
    ...
    for epoch in tqdm(range(MAX_EPOCHS), desc="SAA Training"):
        ...
        # NEW: Log robustness at the specified interval
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            epoch_loss = 0
            model.train() # Start epoch in train mode
            for X_batch, y_batch in train_loader:
    ...
    # 3. NEW: Robustness Evolution Plot
