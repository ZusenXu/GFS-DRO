...
def plot_frame(model, true_boundary_model, X, y, X_perturbed, title, save_path, method):
    ...
    if X_perturbed is not None:
        ...
        sns.kdeplot(x=X_p_np[y_repeated==1, 0], y=X_p_np[y_repeated==1, 1], ax=ax, color='red', fill=True, alpha=0.4, bw_adjust=0.5)
        sns.kdeplot(x=X_p_np[y_repeated==0, 0], y=X_p_np[y_repeated==0, 1], ax=ax, color='blue', fill=True, alpha=0.4, bw_adjust=0.5)
