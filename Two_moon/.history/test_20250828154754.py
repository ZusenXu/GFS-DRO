...
def plot_frame(model, true_boundary_model, X, y, X_perturbed, title, save_path, method):
    ...
    ax.contour(xx, yy, Z_model, levels=[0.5], linewidths=2)
    legend_elements = [Line2D([0], [0], color='green', linestyle='--', lw=3, label=f'Boundary by {method}'), Patch(facecolor='red', alpha=0.4, label='Positive Worst-Case Dist.'), Patch(facecolor='blue', alpha=0.4, label='Negative Worst-Case Dist.')]
