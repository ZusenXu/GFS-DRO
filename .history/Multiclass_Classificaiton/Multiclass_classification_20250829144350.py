...
class SinkhornDROMultiLD(BaseLinearDRO):
    ...
    def _MultiLD_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        for _ in range(10):
            ...
            mean = x_clone + 0.01 * (grads / (2 * self.lambda_param * self.epsilon) - (x_clone - x_original_expanded) / self.epsilon)
            std_dev = torch.sqrt(torch.tensor(0.02, device=DEVICE))
...
def plot_training_loss(all_runs_losses):
...
def plot_robustness_results(all_runs_results, perturbation_levels, epsilon_values):
    plt.style.use('jz.mplstyle')
    plt.figure()
...
if __name__ == '__main__':
    ...
    figure.dpi : 300
    """
    with open("jz.mplstyle", "w") as f:
        f.write(style_content)
    print("Created custom matplotlib style 'jz.mplstyle'")
    ...
    if not os.path.exists(file_path):
        ...
        DRO_BATCH_SIZE = 256
        lam, eps, itr = 10, 0.1, 8 # Hyperparameters
        ...
        all_runs_training_losses = {"MultiLD": [], "LD": [], "Dual": [], "RGO": []}#
        all_runs_eval_results = {"MultiLD": [], "LD": [], "SAA": [], "Dual": [], "RGO": []} #, "RGO": []
        lr = 5e-3
        ...
        for i in range(N_REPEATS):
            ...
            models_to_evaluate = {"SAA": saa_model_wrapper, "Dual": base_dro_model, "LD": LD_dro_model, "RGO": rgo_dro_model, "MultiLD": MultiLD_dro_model} #, "RGO": rgo_dro_model
        ...
        plot_training_loss(all_runs_training_losses)
        plot_robustness_results(all_runs_eval_results, perturbation_levels, epsilon_values)