...
class SinkhornDROWFR(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, epsilon: float = 0.1, lambda_param: float = 1.0, num_samples: int = 10, max_itr: int = 30, learning_rate: float = 0.01, batch_size: int = 64, inner_lr: float = 0.001, inner_itr: int = 100, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
        self.epsilon, self.lambda_param = epsilon, lambda_param
...
class WRM(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, lambda_param: float = 1.0, max_itr: int = 30, learning_rate: float = 0.01, batch_size: int = 64, inner_lr: float = 0.001, inner_itr: int = 100, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(file_path):
        ...
        DRO_BATCH_SIZE = 128
        lam_values = [1.0, 10.0]
        eps_values = [0.5, 0.05, 0.005]
        itr = 1 # Hyperparameters
        ...
        for lam in lam_values:
            for eps in eps_values:
                ...
                for i in range(N_REPEATS):
                    ...
                    perturbation_levels = [0.0, 0.005, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.045, 0.05]
