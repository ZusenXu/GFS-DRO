...
class SinkhornDROWFR(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, epsilon: float = 0.1, lambda_param: float = 1.0, num_samples: int = 10, max_itr: int = 30, learning_rate: float = 0.01, batch_size: int = 64, inner_lr: float = 0.001, inner_itr: int = 200, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
        self.epsilon, self.lambda_param = epsilon, lambda_param
...
class WRM(BaseLinearDRO):
    def __init__(self, input_dim: int, num_classes: int, fit_intercept: bool = True, lambda_param: float = 1.0, max_itr: int = 30, learning_rate: float = 0.01, batch_size: int = 64, inner_lr: float = 0.001, inner_itr: int = 200, device: str = "cpu"):
        self.device = torch.device(device if device == "cuda" and torch.cuda.is_available() else "cpu")
        super().__init__(input_dim, num_classes, fit_intercept)
...
def plot_robustness_results(all_runs_results, perturbation_levels, epsilon_values, lam, eps):
    plt.style.use('jz.mplstyle')
    plt.figure(figsize=(3.5, 2.613), dpi=300, tight_layout=True)
        ...
        for lam in lam_values:
            for eps in eps_values:
                ...
                for i in range(N_REPEATS):
                    ...
                    perturbation_levels = [0.0, 0.008, 0.016, 0.024, 0.032, 0.04]
