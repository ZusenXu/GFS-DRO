...
class SDRO_SVG(BaseLinearDRO):
    ...
    def _svg_sampler(self, x_original_batch: torch.Tensor, y_original_batch: torch.Tensor, model: nn.Module, epoch) -> torch.Tensor:
        ...
        for _ in range(int(self.inner_itr * (epoch + 1)/self.max_itr)):
...
if __name__ == '__main__':
    ...
    resnet_feature_extractor.fc = nn.Sequential(
        nn.Linear(resnet_feature_extractor.fc.in_features, 512),
        ...
        for lam in lam_values:
            for eps in eps_values:
                ...
                for i in range(N_REPEATS):
                    ...
                    print("\n--- Training SDRO_SVG Model ---")
                    SVG_dro_model = SDRO_SVG(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, 
                    ...
                    print("\n--- Training SDRO_WFR Model ---")
                    WFR_dro_model = SDRO_WFR(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_itr=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE, num_samples=16)
                    ...
                    print("\n--- Training SDRO_WGF Model ---")
                    WGF_dro_model = SDRO_WGF(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_itr=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE, num_samples=16)
                    ...
                    print("\n--- Training SDRO_Dual (Vectorized) ---")
                    base_dro_model = SDRO_Dual(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_itr=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE, sample_level=5)
                    ...
                    print("\n--- Training SDRO_RGO Model ---")
                    rgo_dro_model = SDRO_RGO(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_itr=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE, num_samples=16)
