...
class SinkhornDROWFR(BaseLinearDRO):
    ...
    def _WFR_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module, epoch) -> torch.Tensor:
        """
        Generates worst-case samples and their corresponding weights using Langevin dynamics.
        
        This method implements a sampler to find a worst-case distribution by iteratively
        updating a set of particles (samples) to maximize the model's loss, constrained by
        the WFR distance. It now includes a resampling step to handle particles with
        diminishing weights.
        """
        batch_size = x_orig.shape[0]

        # Initialize weights uniformly for all samples
        weights = torch.ones((batch_size, self.num_samples), dtype=torch.float32, device=self.device) / self.num_samples

        # Clone and expand original data for batch processing of samples
        ...
        # Precompute constants for the weight update rule
        wt_lr = self.inner_lr
        weight_exponent = 1 - self.lambda_param * self.epsilon * wt_lr

        # Dynamically adjust the number of inner steps based on the current epoch
        num_steps = int(max(5, self.inner_itr * (epoch + 1) / self.max_itr))
        
        for _ in range(num_steps):
            x_clone.requires_grad_(True)
            
            # Compute gradients of the loss with respect to the samples
            ...
            # Perform a step of Langevin dynamics
...
class SinkhornDROSVG(BaseLinearDRO):
    ...
    def _rbf_kernel_batched_torch(self, particles: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Computes the RBF kernel and its gradient for a batch of particles.
        """
        ...
        # Use median heuristic for bandwidth h
    ...
    def _svg_sampler(self, x_original_batch: torch.Tensor, y_original_batch: torch.Tensor, model: nn.Module) -> torch.Tensor:
        """
        Generates worst-case samples using Stein Variational Gradient Descent (SVGD).
        """
        ...
        # Initialize particles around the original data points
        ...
        for _ in range(self.inner_itr):
            ...
            # Gradient of the log-likelihood of the target distribution
            ...
            # Gradient of the log-prior (Gaussian prior centered at original data)
            grad_log_px = -2 * self.lambda_param * (x_tensor - x_orig_repeated_flat)

            # Total gradient is for the posterior: log p(y|x) + log p(x)
            # The ascent direction is grad(loss) - 2*lambda*(x-x_orig)
            ...
            # Driving force towards higher density
            sum_grad_K = torch.sum(grad_K_x, dim=2)

            # Combine forces for the SVGD update
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, checkpoint_dir: str = 'checkpoints', run_id: int = 0) -> List[float]:
        ...
        for epoch in range(self.max_itr):
            ...
            for x_original_batch, y_original_batch in pbar:
                ...
                # Generate perturbed samples using the SVG sampler
                self.model.eval()
                x_svg_batch = self._svg_sampler(x_original_batch_dev, y_original_batch_dev, self.model)
                y_repeated_batch = y_original_batch_dev.repeat_interleave(self.num_samples, dim=0)
                
                # Update model parameters using the generated samples
                self.model.train()
...
def get_cifar10_dataloaders(batch_size=128):
    ...
    # Add random augmentations for the training transform
    ...
    # Keep the test transform deterministic for consistent evaluation
...
if __name__ == '__main__':
    ...
    INPUT_DIM, NUM_CLASSES = 2048, 10 # Feature dimension from the penultimate layer of ResNet-50
    DRO_BATCH_SIZE = 128
    lam_values = [10]
    eps_values = [0.2, 0.02, 0.002]
    itr = 10 # Number of iterations
    
    N_REPEATS = 5 # Number of repeated experiments
    BASE_SEED = 2022
    ...
    for lam in lam_values:
        for eps in eps_values:
            ...
            for i in range(N_REPEATS):
                ...
                # --- Re-extract features with data augmentation for each run ---
                print("Loading data and extracting features for this run...")
