...
class BaseLinearDRO:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        self.model.eval()
        with torch.no_grad(): return self.model(self._to_tensor(X_val)).cuda().numpy()
...
class SinkhornDROMultiLD(BaseLinearDRO):
    ...
    def _MultiLD_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        x_clone = x_clone.unsqueeze(1).expand(-1, self.num_samples, -1).contiguous().view(-1, INPUT_DIM)
        y_repeated = y_orig.repeat_interleave(self.num_samples, dim=0)
        x_original_expanded = x_orig.unsqueeze(1).expand(-1, self.num_samples, -1).reshape(-1, INPUT_DIM)
        for _ in range(self.inner_itr):
            ...
            std_dev = torch.sqrt(torch.tensor(2*self.inner_lr*self.lambda_param*self.epsilon, device=DEVICE))
            mean_expanded = mean.unsqueeze(1).expand(-1, 1, -1)
            noise = torch.randn_like(mean_expanded) * std_dev.to(mean_expanded.device)
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, checkpoint_dir: str = 'checkpoints', run_id: int = 0) -> List[float]:
        ...
        for epoch in range(self.max_iter):
            ...
            for x_original_batch, y_original_batch in pbar:
                x_original_batch_dev, y_original_batch_dev = x_original_batch.to(self.device), y_original_batch.to(self.device)
                self.model.eval()
                x_rgo_batch = self._MultiLD_sampler(x_original_batch_dev, y_original_batch_dev, self.model)
                y_repeated_batch = y_original_batch_dev.repeat_interleave(self.num_samples, dim=0)
                self.model.train()
                predictions_logits_batch = self.model(x_rgo_batch)
                optimization_loss = nn.CrossEntropyLoss()(predictions_logits_batch, y_repeated_batch)
...
class WDRO(BaseLinearDRO):
    ...
    def _sinha_sampler(self, x_orig: torch.Tensor, y_orig: torch.Tensor, model: nn.Module) -> torch.Tensor:
        ...
        for _ in range(self.inner_itr):
            ...
            grads, = torch.autograd.grad(loss_values, x_clone, grad_outputs=torch.ones_like(loss_values))
            x_clone = x_clone + self.inner_lr * (grads - 2*self.lambda_param*(x_clone - x_orig))
        return x_clone.detach()
    def fit(self, X: np.ndarray, y: np.ndarray, checkpoint_dir: str = 'checkpoints', run_id: int = 0) -> List[float]:
        X, y = self._validate_inputs(X, y)
        dataloader = self._create_dataloader(X, y, batch_size=self.batch_size)
        ...
        for epoch in range(self.max_iter):
            ...
            for x_original_batch, y_original_batch in pbar:
                ...
                optimization_loss.backward()
                optimizer_theta.step()
                comparable_loss = nn.CrossEntropyLoss(reduction='mean')(x_original_batch, y_original_batch)
                epoch_loss_record += comparable_loss.item()
                pbar.set_postfix(optim_loss=optimization_loss.item(), record_loss=comparable_loss.item())
            avg_epoch_loss_record = epoch_loss_record / len(dataloader)
            loss_history.append(avg_epoch_loss_record)
            pbar.set_postfix(optim_loss=optimization_loss.item(), record_loss=avg_epoch_loss_record)
...
def evaluate_model(model_object, test_features_np, test_labels_np, attack_fn, epsilon_list):
    ...
    for epsilon in epsilon_list:
        ...
        for features, labels in tqdm(test_loader, desc=f"Evaluating (epsilon={epsilon:.4f})", leave=False):
            ...
            with torch.no_grad():
                outputs = pytorch_model(perturbed_features.to(DEVICE))
...
def plot_robustness_results(all_runs_results, perturbation_levels, epsilon_values, lam, eps):
    ...
    # Define a color cycle to match the example plot if desired
    # colors = {'SAA': 'gray', 'Base': 'black', 'RGO': 'red', 'LD': 'blue'}
    # linestyles = {'SAA': '--', 'Base': '-.', 'RGO': '-', 'LD': '-'}
    
    for model_name, results_runs in all_runs_results.items():
        if not results_runs:
            continue
            
        # Create a matrix of test errors: (n_runs, n_perturbation_levels)
        errors_matrix = []
        query_keys = [0.0] + epsilon_values[1:]

        for single_run_result in results_runs:
            ordered_errors = [100.0 - single_run_result[key] for key in query_keys]
...
if __name__ == '__main__':
    ...
    if not os.path.exists(file_path):
        ...
        DRO_BATCH_SIZE = 256
        lam_values = [10.0, 100.0]
        eps_values = [0.01, 0.1]
        itr = 10 # Hyperparameters
        
        # --- Experiment Repetition Setup ---
        N_REPEATS = 1  # Number of times to repeat the experiment
        BASE_SEED = 2022
        
        # Data structures to store results from all runs
        
        lr = 5e-3
        # --- Main Experiment Loop ---
        for lam in lam_values:
            for eps in eps_values:
                all_runs_training_losses = {"MultiLD": [], "Dual": [], "RGO": []}# "LD": [],  , "WDRO": []
                all_runs_eval_results = {"MultiLD": [], "SAA": [], "Dual": [], "WDRO": [], "RGO": []} #"LD": [], 
                for i in range(N_REPEATS):
                    ...
                    # LD_dro_model = SinkhornDROLD(INPUT_DIM, NUM_CLASSES, device=DEVICE.type, lambda_param=lam, epsilon=eps, max_iter=itr, learning_rate=lr, batch_size=DRO_BATCH_SIZE, num_samples=16)
                    # LD_loss_history = LD_dro_model.fit(train_features_np, train_labels_np, checkpoint_dir=CHECKPOINT_DIR, run_id=i)
                    # all_runs_training_losses["LD"].append(LD_loss_history)
                    ...
                    #all_runs_training_losses["SAA"].append(saa_loss_history)
                    ...
                    #all_runs_training_losses["WDRO"].append(WDRO_loss_history)
                    ...
                    models_to_evaluate = {"SAA": saa_model_wrapper, "Dual": base_dro_model, "MultiLD": MultiLD_dro_model, "WDRO": WDRO_model, "RGO": rgo_dro_model} #"LD": LD_dro_model, 

                    for name, model_obj in models_to_evaluate.items():
                        print(f"\n--- Evaluating {name} ---")
                        # Evaluate on clean (eps=0) and adversarial (eps > 0)
                ...
                # --- Plotting Final Aggregated Results ---
