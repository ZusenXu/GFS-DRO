...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=1, num_samples_per_point=5):
    """
...
def plot_objective_comparison(histories: Dict[str, list], save_path: str):
    ...
    plt.figure(figsize=(2.613, 2.613), dpi=300, tight_layout=True)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "RGO", "Dual", "WGF", "WRM", "WFR"]#"LD", "SVGD",
    loss_histories = {name: [] for name in ["RGO", "Dual", "WGF", "WFR"]}  # SAA does not use DRO loss "LD", "WRM", "SVGD",
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["WFR", "WRM", "WGF", "RGO", "Dual"] #"LD",  , "SVGD"
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["RGO", "LD", "WGF", "WRM", "WFR"]: #"SVGD",
                model.eval()
        ...
        if imageio and method in ["RGO", "WGF", "WRM", "WFR"]: #"LD", "SVGD",
