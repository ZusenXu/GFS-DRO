...
def visualize_all_boundaries(models: Dict[str, nn.Module], true_boundary_model, X, y, title, save_path):
    ...
    for i, (name, model) in enumerate(models.items()):
        ...
        ax.contour(xx, yy, Z, levels=[0.5], linestyles=linestyles[i % len(linestyles)], linewidths=1.5)
    ...
    legend_elements.extend([Line2D([0], [0], lw=1.5, linestyle=linestyles[i % len(linestyles)], label=name) for i, name in enumerate(models.keys())])
