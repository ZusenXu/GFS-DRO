...
def rgo_sampler(x_original_batch, y_original_batch, model, epoch):
    ...
    inner_steps = int(min(5, 50 * (epoch + 1) / MAX_EPOCHS))
...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-3, inner_steps=200):
...
def plot_wfr_weights_evolution(model, X_train, y_train, output_dir, lr=1e-3, inner_steps=1000):
    """
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["RGO", "LD", "MultiLD", "WDRO", "WFR"]: #"SVGD",
                ...
                if method == "MultiLD":
                    ...
                    epoch_X_p, epoch_weights = wfr_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model)
                else:
