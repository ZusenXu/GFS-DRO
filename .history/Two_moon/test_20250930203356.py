...
def proximal_rejection_sampler(x_original_batch, y_original_batch, model, epoch, inner_lr=0.1, inner_steps=100):
    ...
    def f(x):
        return nn.CrossEntropyLoss(reduction='none')(model(x), y_original_batch)
    ...
    def grad_f_y_eta(x):
        ...
        loss_val = f(x)
        reg_val = torch.sum((x - x_original_batch)**2, dim=1) / (2 * eta)
    ...
    for _ in range(40):
        ...
        proposals = torch.randn_like(final_accepted_samples) * proposal_std + proposal_mean.unsqueeze(1)
        
        proposals_flat = proposals.view(-1, INPUT_DIM)
        y_repeated = y_original_batch.repeat_interleave(NUM_SAMPLES_PER_POINT, dim=0)
        f_X = f(proposals_flat).view(batch_size, NUM_SAMPLES_PER_POINT)
        norm_sq_X = torch.sum((proposals - x_original_batch.unsqueeze(1))**2, dim=2)
        f_y_eta_X = f_X + norm_sq_X / (2 * eta)

        term1 = f_w.unsqueeze(1)
        term2 = torch.sum(grad_f_w.unsqueeze(1) * (proposals - w.unsqueeze(1)), dim=2)
        term3 = (M/2) * torch.sum((proposals - w.unsqueeze(1))**2, dim=2)
        term4 = (1/(2*eta)) * torch.sum((proposals - x_original_batch.unsqueeze(1))**2, dim=2)
        const_term = ((1 - ALPHA) * DELTA) / 2
        
        h_1_y_w_X = term1 + term2 - term3 + term4 - const_term
        ...
        final_accepted_samples[newly_accepted_mask] = proposals[newly_accepted_mask]
