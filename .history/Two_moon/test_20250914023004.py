...
MMD_NOISE_STD = 1e-4        # Noise std for Langevin dynamics in MMD sampler
...
OUTPUT_DIR = "two_moons_test"
OUTPUT_DIR_PDF = "two_moons_test_pdf" # New directory for PDFs
...
# =============================================================================
# --- MODIFIED: MMD-DRO SAMPLER IMPLEMENTATION with ADAPTIVE BANDWIDTH ---
# =============================================================================
def rbf_kernel(X, Y):
    """
    Computes the Gaussian (RBF) kernel matrix with an adaptive bandwidth
    set by the median heuristic.
    X: [m, d] tensor
    Y: [m, d] tensor
    Returns: [m, m] kernel matrix
    """
    X_sq = torch.sum(X**2, dim=1, keepdim=True)
    Y_sq = torch.sum(Y**2, dim=1, keepdim=True)
    XY = torch.matmul(X, Y.T)
    
    dist_sq = X_sq - 2*XY + Y_sq.T
    dist_sq = dist_sq.clamp(min=0)
    
    # --- Median Heuristic for bandwidth ---
    n_points = X.shape[0]
    # Flatten the pairwise distances to compute the median
    all_dist_sq = dist_sq.flatten()

    if all_dist_sq.numel() > 1:
        # Calculate h_sq (h^2) using the median heuristic
        h_sq = 0.5 * torch.median(all_dist_sq) / torch.log(torch.tensor(n_points + 1.0, device=X.device))
    else:
        h_sq = torch.tensor(1.0, device=X.device)

    # Ensure h_sq is not too small to avoid numerical instability
...
def mmd_dro_sampler(x_original_batch, y_original_batch, model, epoch, lr=inner_lr, inner_steps=inner_steps):
    """
    Generates worst-case samples for DRO using a stochastic MMD gradient flow.
    This is an interacting particle system with Langevin dynamics.
    """
    ...
    # 2. MMD gradient flow inner loop
    for _ in range(int(max(5, inner_steps * (epoch + 1) / MAX_EPOCHS))):
        # Step A: Compute loss gradient for each particle ∇f(x_j)
        ...
        # Step B: Compute the m x m kernel matrix K_ij = k(x_i, x_j) with adaptive bandwidth
        kernel_matrix = rbf_kernel(x_clone, x_clone)
        
        # Step C: Compute the velocity field v_i = Σ_j k(x_i, x_j) * ∇f(x_j)
        velocity = torch.matmul(kernel_matrix, loss_grads)
        
        # Step D: Add random noise for Langevin dynamics to break symmetry
...
def visualize_all_boundaries(models: Dict[str, nn.Module], true_boundary_model, X, y, title, save_path_png):
    ...
    colors = [
        ...
        "#951699",   
        "#1C7414EF",
        "#1f77b4" # Adding a color for MMD
    ]
    for i, (name, model) in enumerate(models.items()):
        model.eval()
        with torch.no_grad():
            Z = model(grid).argmax(dim=1).cpu().numpy().reshape(xx.shape)
        ax.contour(xx, yy, Z, levels=[0.5], colors=[colors[i % len(colors)]], linewidths=1.5)
    ideal_color = 'black'
    ideal_linewidth = 1.5
    ...
    legend_elements.extend([Line2D([0], [0], lw=1.5, color=colors[i % len(colors)], label=name) for i, name in enumerate(models.keys())])
...
def plot_robustness_comparison(history: Dict[str, Dict[str, list]], save_dir_png: str, save_dir_pdf: str):
    ...
    for attack in attack_types:
        ...
        plt.xlabel("Epoch"); plt.ylabel("Accuracy"); plt.title(f"Testing accuracy under {attack} Attack")
        plt.legend(loc='upper left', bbox_to_anchor=(1.02, 1)); plt.grid(False); plt.ylim(0, 1.05)
        plt.gca().xaxis.set_major_locator(plt.MaxNLocator(integer=True))
...
if __name__ == '__main__':
    ...
    # INTEGRATION: Add "MMD" to the list of models to train and evaluate
    model_names = ["SAA", "RGO", "Dual", "WGF", "WRM", "WFR", "MMD"]
    loss_histories = {name: [] for name in ["RGO", "Dual", "WGF", "WFR", "MMD"]}
    ...
    for epoch in tqdm(range(MAX_EPOCHS), desc="SAA Training"):
        ...
        for X_batch, y_batch in train_loader:
            ...
            loss.backward(); optimizer_saa.step()
    ...
    # --- DRO Models Training ---
    # INTEGRATION: Add "MMD" to the training loop
    dro_methods_to_train = ["MMD", "WFR", "WRM", "WGF", "RGO", "Dual"]
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                ...
                else: # Handle all other particle/sampler-based methods
                    model.eval()
                    if method == "WGF":
                    ...
                    # --- NEW: MMD SAMPLER CALL ---
                    elif method == "MMD":
                        X_perturbed = mmd_dro_sampler(X_batch, y_batch, model, epoch)
                    
                    model.train()
                    if X_perturbed is not None:
                        # Determine label repetition factor
                        num_repeats = NUM_SAMPLES_PER_POINT if method != "WRM" else 1
                        y_repeated = y_batch.repeat_interleave(num_repeats)
            ...
            if (epoch + 1) % LOG_EPOCH_INTERVAL == 0 :
                if method != "WRM": # WRM loss is not directly comparable
            ...
            # INTEGRATION: Add "MMD" to the list of methods to generate GIFs for
            if method in ["RGO", "WGF", "WRM", "WFR", "MMD"]:
                model.eval()
                epoch_weights = None
                epoch_X_p = None
                if method == "WGF":
                ...
                # --- NEW: MMD SAMPLER FOR VISUALIZATION ---
                elif method == "MMD":
        ...
        # INTEGRATION: Add "MMD" to GIF generation
        if imageio and method in ["RGO", "WGF", "WRM", "WFR", "MMD"]:
