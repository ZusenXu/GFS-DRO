...
MMD_NOISE_STD = 1e-3 # Noise for MMD Langevin Dynamics
...
def mmd_dro_sampler(x_original_batch, y_original_batch, model, epoch, kernel_fn, lr=0.01, inner_steps=10, 
                    NUM_SAMPLES_PER_POINT=NUM_SAMPLES_PER_POINT, INPUT_DIM=2, MAX_EPOCHS=100, LAMBDA_PARAM=1.0, MMD_NOISE_STD=0.01):
    """
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating MMD frames"):
        ...
        if step % 5 == 0: # Plot every 5 steps
             ...
             plot_frame_samples_with_forces(
                 ...
                 force1=0.01*loss_grads,
                 force2=-0.01*mmd_force
             )
             gif_filenames.append(frame_path)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "Dual", "WGF", "WRM", "WFR", "MMD"] #"RGO", 
    loss_histories = {name: [] for name in ["Dual", "WGF", "WFR", "MMD"]}  # SAA does not use DRO loss "RGO", 
    ...
    optimizer_saa = optim.Adam(saa_model.parameters(), lr=LR, weight_decay=1e-4)
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["WFR", "WRM", "WGF", "Dual", "MMD"] #"RGO", 
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                    ...
                    model.eval()
                    X_perturbed = mmd_dro_sampler(X_batch, y_batch, model, epoch, imq_kernel_and_grad)
            ...
            if method in ["RGO", "LD", "WGF", "WRM", "WFR", "SVGD", "MMD"]: 
                ...
                if method == "WGF":
                    ...
                    epoch_X_p = mmd_dro_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, epoch, imq_kernel_and_grad)
                elif method == "WFR":
