...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=10):
...
def plot_frame_samples(model, true_boundary_model, X, y, X_perturbed, title, save_path, method, epoch, weights=None):
    """
    Plots decision boundary, data points, and connects original points (X)
    to their corresponding perturbed samples (X_perturbed). The transparency
    of perturbed points is controlled by the `weights` variable.
    
    NOTE: The `true_boundary_model` parameter is not used in this function.
    """
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
    ...
    if X_perturbed is not None:
        ...
        # 2b. Scale weights to a visible alpha range [0.1, 1.0]
    ...
    legend_elements = [
        ...
        Line2D([0], [0], linestyle='None', marker='o', color='darkorange', label='Positive Data', markersize=5, markeredgecolor='k', linewidths=0.5),
        Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', label='Negative Data', markersize=5, markeredgecolor='k', linewidths=0.5),
        Line2D([0], [0], linestyle='None', marker='o', color='darkorange', label='Perturbed Positive', markersize=5, edgecolors="gray", linewidths=0.2, s=15),
        Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', label='Perturbed Negative', markersize=5, edgecolors="gray", linewidths=0.2, s=15)
    ]
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["RGO", "LD", "MultiLD", "WDRO", "WFR"]: #"SVGD", 
                ...
                if method == "MultiLD":
                    ...
                    epoch_X_p, epoch_weights = wfr_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, lr=1e-2, inner_steps=50)
                else:
