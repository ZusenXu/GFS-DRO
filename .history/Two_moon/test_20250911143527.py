...
def rgo_sampler(x_original_batch, y_original_batch, model, epoch):
    ...
    inner_steps = int(min(5, inner_steps * (epoch + 1) / MAX_EPOCHS))
...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=inner_lr, inner_steps=inner_steps):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=inner_lr, inner_steps=inner_steps):
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=inner_lr, inner_steps=inner_steps):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
        loss_values = nn.CrossEntropyLoss(reduction='none')(model(x_clone), y_original_batch)
        grads, = torch.autograd.grad(loss_values, x_clone, grad_outputs=torch.ones_like(loss_values))
