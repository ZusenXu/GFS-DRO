...
MMD_NOISE_STD = 1e-3        # Noise std for Langevin dynamics in MMD sampler
...
def rbf_kernel_and_grad(X, Y):
    """
    Computes the Gaussian (RBF) kernel matrix and its gradient w.r.t. X.
    The bandwidth is set adaptively by the median heuristic.
    X: [m, d] tensor
    Y: [n, d] tensor
    Returns:
        K: [m, n] kernel matrix
        grad_K: [m, n, d] gradient tensor where grad_K[i, j, :] = ∇_x_i k(x_i, y_j)
    """
    m, d = X.shape
    n, _ = Y.shape
...
def mmd_dro_sampler(x_original_batch, y_original_batch, model, epoch, lr=inner_lr, inner_steps=inner_steps):
    ...
    based on the correct gradient of the MMD-DRO objective.
    """
...
if __name__ == '__main__':
    ...
    X_train, y_train = create_dataset(n_samples=N_SAMPLES_TRAIN, noise=0.1, imbalance_ratio=0.5, random_state=SEED)
