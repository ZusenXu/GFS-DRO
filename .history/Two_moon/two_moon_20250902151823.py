...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=1e-4, inner_steps=100):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=1e-3, inner_steps=50):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
        loss_values = nn.CrossEntropyLoss(reduction='none')(model(x_clone), y_repeated)
        grads, = torch.autograd.grad(loss_values, x_clone, grad_outputs=torch.ones_like(loss_values))
...
def plot_frame_samples(model, true_boundary_model, X, y, X_perturbed, title, save_path, method, epoch):
    ...
    if X_perturbed is not None:
        ...
        # 2c. 绘制所有扰动点 (使用 'x' 标记)
        ax.scatter(X_p_np[y_repeated==1, 0], X_p_np[y_repeated==1, 1], c='darkorange', marker='x', label='Perturbed Positive', s=25)
        ax.scatter(X_p_np[y_repeated==0, 0], X_p_np[y_repeated==0, 1], c='dodgerblue', marker='x', label='Perturbed Negative', s=25)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "RGO", "LD", "Dual", "MultiLD"]
    loss_histories = {name: [] for name in ["RGO", "LD", "Dual", "MultiLD"]}
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["MultiLD", "RGO", "LD", "Dual"]
    ...
    for method in dro_methods_to_train:
        ...
        if imageio and method in ["RGO", "LD", "MultiLD"]:
