...
def wfr_sampler(x_original_batch, y_original_batch, model, epoch, lr=inner_lr, inner_steps=inner_steps*4):
    ...
    wt_lr = lr * wfr_times
    weight_exponent = 1 - EPSILON * wt_lr
    ...
    for _ in range(int(max(5, inner_steps * (epoch + 1) / MAX_EPOCHS))):
        ...
        mean = x_clone + lr * (grads - 2 * LAMBDA_PARAM * (x_clone - x_original_expanded))/LAMBDA_PARAM
        std_dev = torch.sqrt(torch.tensor(2*lr*EPSILON, device=device))
        ...
        with torch.no_grad():
            ...
            weights = weights ** weight_exponent * torch.exp(new_loss * wt_lr / LAMBDA_PARAM)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "MMD", "Dual", "WGF", "WRM", "WFR"]#"LD", "SVGD","RGO", 
    loss_histories = {name: [] for name in ["Dual", "WGF", "WFR", "MMD"]}  # SAA does not use DRO loss "LD", "WRM", "SVGD", "RGO", 
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["MMD", "WFR", "WRM", "WGF", "Dual"] #"LD",  , "SVGD" "RGO", 
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["RGO", "LD", "WGF", "WRM", "WFR", "MMD"]: #"SVGD",
                model.eval()
        ...
        if imageio and method in ["RGO", "WGF", "WRM", "WFR", "MMD"]: #"LD", "SVGD",
