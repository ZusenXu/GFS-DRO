...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=100, num_samples_per_point=5):
    """
    Visualizes perturbation, creates a GIF, and now also tracks the objective function value.
    ...
    gif_filenames = []
    objective_history = []  # To store the objective value at each step
...
def plot_objective_comparison(histories: Dict[str, list], save_path: str):
    """
    Plots the evolution of the perturbation objective value for different methods.
    """
...
if __name__ == '__main__':
    ...
    comparison_histories = {"WFR": wfr_history, "MultiLD": multild_history}
    plot_objective_comparison(
        comparison_histories,
        os.path.join(OUTPUT_DIR, "saa_perturb_objective_comparison.png")
    )
