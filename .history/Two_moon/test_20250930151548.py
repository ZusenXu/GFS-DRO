...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.001, inner_steps=500, adagrad_hist_decay=0.1):
    ...
    if method == "SVGD":
        ...
        # For RGO visualization, we start from the original points and find the optimal perturbation
        if method == 'RGO':
             particles = x_original_expanded.clone().detach().requires_grad_(True)
        else:
             particles = x_original_expanded.clone()
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
        current_particles_flat = particles.view(-1, D)
        with torch.no_grad():
            loss_values = criterion(saa_model(current_particles_flat), y_repeated)
            dist_sq = torch.sum((current_particles_flat - x_original_expanded)**2, dim=1)
            objective_per_sample = loss_values - LAMBDA_PARAM * dist_sq
            if method == 'WFR':
        ...
        if method == "SVGD":
            ...
            # This visualizes the optimization part of RGO, not the final sampling
            grad_total = -grads.view(-1, D) / LAMBDA_PARAM + 2 * (particles.view(-1, D) - x_original_expanded)
            particles -= lr * grad_total # Using a different sign convention here for descent
        else: 
        ...
        if step % 10 == 0:
            frame_path = os.path.join(frames_dir, f"frame_{step:03d}.png")
            # For RGO, we visualize the single particle trajectory during optimization
            if method == 'RGO':
                plot_frame_samples(saa_model, X_train, y_train, particles.view(-1,D), f"Perturbation Process ({method})",
                                frame_path, "SAA", f"Step: {step+1}", weights=None) # No weights for RGO optimization viz
            else:
                 plot_frame_samples(saa_model, X_train, y_train, particles.view(-1,D), f"Perturbation Process ({method})",
...
if __name__ == '__main__':
    ...
    for method in all_methods:
        # Note: For RGO, plot_perturbation_process visualizes the optimization part, not the sampling part.
