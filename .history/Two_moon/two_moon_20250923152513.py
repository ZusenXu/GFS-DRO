...
def plot_perturbation_process_kernel_force(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=200, adagrad_hist_decay=0.9):
    ...
    if method == "MMD":
        ...
        for step in tqdm(range(inner_steps), desc=f"Generating {method} force frames"):
            # Objective Calculation (same as Wasserstein)
            with torch.no_grad():
...
def plot_objective_comparison(histories: Dict[str, list], save_path_png: str):
    ...
    for name, history in histories.items():
        if history: # Plot only if history is not empty
            plt.plot(history, label=name, linewidth=1.5)
    plt.xlabel("iteration")
    plt.ylabel("loss") # More general label
...
if __name__ == '__main__':
    ...
    wgf_forces_history = plot_perturbation_process_wasserstein_forces(saa_model, X_train, y_train, 'WGF', OUTPUT_DIR, lr=inner_lr, inner_steps=300)
    wfr_forces_history = plot_perturbation_process_wasserstein_forces(saa_model, X_train, y_train, 'WFR', OUTPUT_DIR, lr=inner_lr, inner_steps=300)
    wrm_forces_history = plot_perturbation_process_wasserstein_forces(saa_model, X_train, y_train, 'WRM', OUTPUT_DIR, lr=inner_lr, inner_steps=300)
    
    # Capture objective history from the kernel force visualization function
    svgd_forces_history = plot_perturbation_process_kernel_force(saa_model, X_train, y_train, 'SVGD', OUTPUT_DIR, lr=inner_lr, inner_steps=300)
    plot_perturbation_process_kernel_force(saa_model, X_train, y_train, 'MMD', OUTPUT_DIR, lr=inner_lr, inner_steps=300)

    # Plot objective value comparison including SVGD
    comparison_histories = {
        "WFR": wfr_forces_history,
        "WGF": wgf_forces_history,
        "WRM": wrm_forces_history,
        "SVGD": svgd_forces_history # Added SVGD history for comparison
    }
    plot_objective_comparison(
        comparison_histories,
        os.path.join(OUTPUT_DIR, "saa_perturb_objective_comparison.pdf")
    )
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["SVGD", "WFR", "WRM", "WGF", "Dual"] #"MMD", 
