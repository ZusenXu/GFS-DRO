...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=1e-3, inner_steps=100):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
...
def svgd_sampler_independent(
    ...
    for i in range(len(x_original_batch)):
        ...
        particles = particles + np.random.randn(*particles.shape)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "RGO", "LD", "Dual", "MultiLD", "Sinha", "SVGD"]
    loss_histories = {name: [] for name in ["RGO", "LD", "Dual", "MultiLD", "Sinha", "SVGD"]}  # SAA does not use DRO loss
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["SVGD", "Sinha", "MultiLD", "RGO", "LD", "Dual"]
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["SVGD", "RGO", "LD", "MultiLD", "Sinha"]:
                model.eval()
                if method == "MultiLD":
        ...
        if imageio and method in ["RGO", "LD", "MultiLD", "Sinha", "SVGD"]:
