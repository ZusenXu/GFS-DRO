...
# --- NEW FUNCTION START ---
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.005, inner_steps=1000, num_samples_per_point=5):
    """
    Visualizes the perturbation process of WFR or MultiLD for the SAA model.
    Generates frames and compiles them into a GIF.
    """
    ...
    # --- 3. Iterative Perturbation and Frame Generation ---
    for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
        x_clone.requires_grad_(True)
        
        # Calculate gradients w.r.t. input using the SAA model
        loss_values = nn.CrossEntropyLoss(reduction='none')(saa_model(x_clone), y_repeated)
        grads, = torch.autograd.grad(loss_values, x_clone, grad_outputs=torch.ones_like(loss_values))
        x_clone = x_clone.detach()
        ...
        # Update weights for WFR
        if method == 'WFR':
            with torch.no_grad():
                new_loss = nn.CrossEntropyLoss(reduction='none')(saa_model(x_clone), y_repeated).view(batch_size, -1) - \
                           2 * LAMBDA_PARAM * torch.sum((x_clone - x_original_expanded) ** 2, dim=1).view(batch_size, -1)
        ...
        plot_frame_samples(
            model=saa_model,
            true_boundary_model=None,
            X=X_train,
            y=y_train,
            X_perturbed=x_clone,
            title=f"SAA Boundary with {method} Perturbations",
            save_path=frame_path,
            method="SAA", # The boundary is from SAA
            epoch=f"Step: {step+1}", # Use epoch parameter to display step
            weights=weights
        )
...
if __name__ == '__main__':
    ...
    # --- NEW: Visualize SAA Perturbation Process ---
    plot_perturbation_process(saa_model, X_train, y_train, 'WFR', OUTPUT_DIR)
    plot_perturbation_process(saa_model, X_train, y_train, 'MultiLD', OUTPUT_DIR)
    ...
    for method in dro_methods_to_train:
        ...
        # --- MODIFIED: Create subdirectories for frames ---
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["RGO", "LD", "MultiLD", "WDRO", "WFR"]: #"SVGD",
                ...
                plot_frame_samples(model, true_boundary_model, X_train, y_train, epoch_X_p, f"Worst-case Samples({method})", samples_frame_path, method, epoch, weights=epoch_weights)
