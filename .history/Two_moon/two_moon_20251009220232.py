...
def rbf_kernel_batched_torch(particles):
    ...
    h_squared = 1e-2 #median_sq_dist / (torch.log(torch.tensor(S, device=device)) + 1e-8) + 1e-2

    K = torch.exp(-sq_dist / (2 * h_squared))

    diff = particles.unsqueeze(2) - particles.unsqueeze(1)
    grad_K_x = -diff / h_squared * K.unsqueeze(-1) #.unsqueeze(-1)
