...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.001, inner_steps=500, adagrad_hist_decay=0.1):
    ...
    if method == 'RGO':
        ...
        weights = None
        if method == "SVGD":
            particles = X_train_torch.unsqueeze(1).repeat(1, S, 1) + torch.randn(B, S, D, device=device) *0.5
            hist_grad = torch.zeros_like(particles)
        else:
            particles = x_original_expanded.clone()
            if method == 'WFR':
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "SVGD", "MMD", "Dual", "WGF", "WRM", "WFR", "RGO"]
    loss_histories = {name: [] for name in ["SVGD", "Dual", "WGF", "WFR", "MMD", "RGO"]}
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["SVGD", "WRM", "Dual", "RGO"] #"MMD", "WFR", "WGF", 
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if method in ["SVGD", "WGF", "WRM", "WFR", "MMD", "RGO"]:
                model.eval()
                epoch_weights = None 
                if method == "WGF":
        ...
        if imageio and method in ["WGF", "WRM", "WFR", "MMD", "SVGD", "RGO"]:
