...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=400, num_samples_per_point=5):
    """
    Visualizes perturbation for WFR, MultiLD, and WDRO.
    ...
    # For WDRO, we want to visualize multiple trajectories, so we adapt it to the multi-particle setting
    num_particles = num_samples_per_point if method != "WDRO_single" else 1 # Allow single particle for true WDRO viz
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
        with torch.no_grad():
            ...
            if method == 'WFR':
                ...
                # For MultiLD and WDRO, use the simple mean as weights are uniform
        ...
        if method == 'WDRO':
            # WDRO is deterministic gradient ascent (no noise)
            x_clone += update_direction
        else: # WFR and MultiLD use Langevin Dynamics (with noise)
...
# --- 5. NEW Plotting function for Objective Comparison ---
def plot_objective_comparison(histories: Dict[str, list], save_path: str):
    ...
    plt.title("Perturbation Objective Value vs. Iteration Step")
    plt.legend()
    plt.grid(True, linestyle='--', alpha=0.6)
    plt.savefig(save_path)
    plt.close()
    print(f"Objective comparison plot saved to {save_path}")
...
if __name__ == '__main__':
    ...
    multild_history = plot_perturbation_process(saa_model, X_train, y_train, 'MultiLD', OUTPUT_DIR)
    wdro_history = plot_perturbation_process(saa_model, X_train, y_train, 'WDRO', OUTPUT_DIR)

    # --- Generate the New Comparison Plot ---
    comparison_histories = {
        "WFR": wfr_history,
        "MultiLD": multild_history,
        "WDRO": wdro_history
    }
