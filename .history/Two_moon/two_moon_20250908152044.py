...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=50):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=50):
    weights = np.ones((x_original_batch.shape[0], NUM_SAMPLES_PER_POINT), dtype=np.float32) / NUM_SAMPLES_PER_POINT
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    x_clone = x_clone.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1).contiguous().view(-1, INPUT_DIM)
    y_repeated = y_original_batch.repeat_interleave(NUM_SAMPLES_PER_POINT, dim=0)
    x_original_expanded = x_original_batch.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1).reshape(-1, INPUT_DIM)
    for _ in range(inner_steps):
        ...
        noise = torch.randn_like(mean_expanded) * std_dev
        x_clone = (mean_expanded + noise).view(-1, INPUT_DIM)
        weights = weights**(1-LAMBDA_PARAM*EPSILON*lr) * torch.exp(-nn.CrossEntropyLoss(reduction='none')(model(x_clone), y_repeated).detach().view(-1, NUM_SAMPLES_PER_POINT).cpu().numpy() * lr)
        weights = weights / np.sum(weights, axis=1, keepdims=True)
    return x_clone.detach(), weights
def sinha_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=50):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
        loss_values = nn.CrossEntropyLoss(reduction='none')(model(x_clone), y_original_batch)
        grads, = torch.autograd.grad(loss_values, x_clone, grad_outputs=torch.ones_like(loss_values))
...
def plot_frame_samples(model, true_boundary_model, X, y, X_perturbed, title, save_path, method, epoch):
    """
    绘制决策边界、数据点，并将每个原始点(X)与其对应的多个扰动点(X_perturbed)连接。
    
    注意: 参数 'true_boundary_model' 在此函数中未被使用。
    """
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
    ...
    # 2. 如果存在扰动数据，则绘制它们并执行“一对多”连接
    if X_perturbed is not None:
        X_p_np = X_perturbed.cpu().numpy()
        
        # 2a. 计算每个原始点对应的扰动点数量 (NUM)
        num_repeats = X_perturbed.shape[0] // X.shape[0]
        
        # 2b. 创建重复的标签数组，用于为所有扰动点着色
        y_repeated = np.repeat(y, num_repeats)
        ...
        # 2d. 循环遍历每个原始点，并将其与对应的 NUM 个扰动点连接
        for i in range(X.shape[0]):
            # 获取当前原始点
            original_point = X[i]
            
            # 定位该点对应的一组扰动点在 X_p_np 中的索引范围
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "RGO", "Dual", "MultiLD", "WDRO", "SVGD"]#"LD", 
    loss_histories = {name: [] for name in ["RGO", "Dual", "MultiLD", "WDRO", "SVGD"]}  # SAA does not use DRO loss "LD", 
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["WFR", "WDRO", "MultiLD", "RGO", "Dual", "SVGD"] #"LD", 
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                    ...
                    model.eval()
                    X_perturbed, weights = wfr_sampler(X_batch, y_batch, model, lr=1e-2, inner_steps=50)
                    y_repeated = y_batch.repeat_interleave(NUM_SAMPLES_PER_POINT)
                    model.train()
                    predictions = model(X_perturbed)
                    loss_values = nn.CrossEntropyLoss(reduction='none')(predictions, y_repeated)
                    weighted_loss = (loss_values.view(-1, NUM_SAMPLES_PER_POINT) * torch.from_numpy(weights).to(DEVICE)).sum(dim=1).mean()
                    record_loss = compute_sinkhorn_loss(model(sinkhorn_base_sampler(X_batch, m_samples=NUM_SAMPLES_PER_POINT)), y_repeated, NUM_SAMPLES_PER_POINT, LAMBDA_PARAM * EPSILON)
                    loss = weighted_loss
            ...
            if method in ["SVGD", "RGO", "LD", "MultiLD", "WDRO", "WFR"]:
                model.eval()
                if method == "MultiLD":
                    ...
                    epoch_X_p, _ = wfr_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, lr=1e-2, inner_steps=50)
                else:
                ...
                plot_frame_samples(model, true_boundary_model, X_train, y_train, epoch_X_p, f"Worst-case Samples({method})", frame_path.replace(".png", "_samples.png"), method, epoch)
        ...
        if imageio and method in ["RGO", "MultiLD", "WDRO", "SVGD", "WFR"]: #"LD", 
