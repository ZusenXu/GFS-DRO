...
def rbf_kernel_batched_torch(particles):
    ...
    h_squared =  median_sq_dist / (torch.log(torch.tensor(S, device=device)) + 1e-8) + 1e-2
...
def svgd_sampler(
    ...
    particles += torch.randn_like(particles) *0.5 #* np.sqrt(EPSILON / 2)
...
# --- NEW SAMPLER FROM PROXIMAL_SAMPLER.PDF ---

def accelerated_gradient_method(g, x0, L, mu, rho, max_iter=100):
    """
    Implementation of Algorithm 3 from proximal_sampler.pdf.
    Finds an approximate stationary point for a given function g.
    """
    y_k = x0.clone()
    x_k = x0.clone()
    t_k = 1
    ...
    for _ in range(max_iter):
        ...
        x_k_plus_1 = y_k - (1/L) * grad_y
        
        t_k_plus_1 = (1 + math.sqrt(1 + 4 * t_k**2)) / 2 # Simplified Nesterov momentum
        y_k_plus_1 = x_k_plus_1 + ((t_k - 1)/t_k_plus_1) * (x_k_plus_1 - x_k)
        ...
        grad_check, _ = g(y_k)
        if torch.norm(grad_check.view(x0.shape[0], -1), p=2, dim=1).max() <= rho:
            return y_k
            
    return y_k

def accelerated_gradient_method_with_history(g, x0, L, mu, rho, max_iter=100):
    """
    Modified version of Algorithm 3 to return optimization history for visualization.
    """
    y_k = x0.clone()
    x_k = x0.clone()
    t_k = 1
    history = [y_k.clone().detach()]
    ...
    for _ in range(max_iter):
        ...
        t_k_plus_1 = (1 + math.sqrt(1 + 4 * t_k**2)) / 2
        y_k_plus_1 = x_k_plus_1 + ((t_k - 1)/t_k_plus_1) * (x_k_plus_1 - x_k)
        
        y_k = y_k_plus_1
        x_k = x_k_plus_1
        t_k = t_k_plus_1
        history.append(y_k.clone().detach())
...
def proximal_rejection_sampler(x_original_batch, y_original_batch, model, epoch, inner_lr=0.1, inner_steps=100):
    """
    Implementation of Algorithm 2 from proximal_sampler.pdf.
    """
    batch_size = x_original_batch.shape[0]
    
    def f(x):
        # Use a closure to capture the model and y_original_batch
        y_b = y_original_batch
        if x.shape[0] != y_b.shape[0]:
            y_b = y_b.repeat_interleave(x.shape[0] // y_b.shape[0], dim=0)
        return nn.CrossEntropyLoss(reduction='none')(model(x), y_b)
    ...
    M = (L_SMOOTHNESS**(2/(ALPHA+1))) / (((ALPHA+1)*DELTA)**((1-ALPHA)/(ALPHA+1)))
    
    d_dim = x_original_batch.shape[1]
    # Ensure eta is less than 1/(M*d) for valid proposal variance
    eta = 0.99 / (M * d_dim) 

    def grad_f_y_eta(x):
        x.requires_grad_(True)
        # Detach original batch to prevent grads flowing back
        x_orig = x_original_batch.detach()
        loss_val = f(x)
        reg_val = torch.sum((x - x_orig)**2, dim=1) / (2 * eta)
        objective = loss_val + reg_val
        # Summing is necessary for scalar backward
        grads, = torch.autograd.grad(objective.sum(), x)
        x.requires_grad_(False)
        return grads, objective.detach()

    # --- 1. Find approximate stationary point w ---
    L_g = 1/eta + M
    mu_g = max(0, 1/eta - M)
    rho = math.sqrt(M * d_dim)
    w_initial = x_original_batch.clone().detach()
    w = accelerated_gradient_method(grad_f_y_eta, w_initial, L_g, mu_g, rho, max_iter=inner_steps)

    # --- 2. Rejection Sampling ---
    final_accepted_samples = torch.zeros((batch_size, NUM_SAMPLES_PER_POINT, INPUT_DIM), device=DEVICE)
    active_flags = torch.ones((batch_size, NUM_SAMPLES_PER_POINT), dtype=torch.bool, device=DEVICE)
    
    w.requires_grad_(True)
    f_w = f(w)
    grad_f_w, = torch.autograd.grad(f_w.sum(), w)
    w.requires_grad_(False)

    proposal_variance = eta / (1 - eta * M)
    proposal_std = math.sqrt(proposal_variance)
    
    s = grad_f_w + (1/eta)*(w - x_original_batch)
    proposal_mean = w - (eta / (1-eta*M))*s

    for _ in range(40): # Max rejection trials
        if not active_flags.any():
            break

        # Generate samples from active indices only
        num_active = active_flags.sum()
        proposals = torch.randn((num_active, INPUT_DIM), device=DEVICE) * proposal_std 
        
        # Expand proposal_mean to match active proposals
        expanded_mean = proposal_mean.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1)[active_flags]
        proposals += expanded_mean
        
        current_proposals = torch.zeros_like(final_accepted_samples)
        current_proposals[active_flags] = proposals

        # Calculate acceptance probability
        f_y_eta_X = torch.zeros_like(active_flags, dtype=torch.float32)
        h_1_y_w_X = torch.zeros_like(active_flags, dtype=torch.float32)

        if num_active > 0:
            f_X = f(proposals).view(num_active)
            
            x_orig_expanded = x_original_batch.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1)[active_flags]
            norm_sq_X = torch.sum((proposals - x_orig_expanded)**2, dim=1)
            
            f_y_eta_X_active = f_X + norm_sq_X / (2 * eta)
            f_y_eta_X[active_flags] = f_y_eta_X_active

            # h_1_y_w(X)
            w_expanded = w.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1)[active_flags]
            grad_f_w_expanded = grad_f_w.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1)[active_flags]
            f_w_expanded = f_w.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT)[active_flags]

            term1 = f_w_expanded
            term2 = torch.sum(grad_f_w_expanded * (proposals - w_expanded), dim=1)
            term3 = (M/2) * torch.sum((proposals - w_expanded)**2, dim=1)
            term4 = (1/(2*eta)) * norm_sq_X
            const_term = ((1 - ALPHA) * DELTA) / 2
            
            h_1_y_w_X_active = term1 + term2 - term3 + term4 - const_term
            h_1_y_w_X[active_flags] = h_1_y_w_X_active

        log_acceptance_prob = -f_y_eta_X + h_1_y_w_X
        acceptance_probs = torch.exp(torch.clamp(log_acceptance_prob, max=0))

        rand_vals = torch.rand_like(acceptance_probs)
        newly_accepted_mask = (rand_vals < acceptance_probs) & active_flags
        
        final_accepted_samples[newly_accepted_mask] = current_proposals[newly_accepted_mask]
        active_flags[newly_accepted_mask] = False

    # Fallback for non-accepted points
    final_accepted_samples[active_flags] = proposal_mean.unsqueeze(1).expand(-1, NUM_SAMPLES_PER_POINT, -1)[active_flags]

    return final_accepted_samples.view(-1, INPUT_DIM)
...
def plot_frame_samples(model, X, y, X_perturbed, title, save_path, method, epoch, weights=None):
    ...
    if X_perturbed is not None:
        ...
        if X_perturbed.shape[0] == X.shape[0]:
            num_repeats = 1
            y_repeated = y
        else:
            num_repeats = X_perturbed.shape[0] // X.shape[0]
...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path_png):
    ...
    colors = [
        ...
        "#1C7414EF",
        "#FF7F00",
        "#2E8B57" # Added color for the new method
    ]
    linestyles = ['--', ':', '-.', '--']
...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.001, inner_steps=500, adagrad_hist_decay=0.1):
    ...
    if method == "ProximalRejection":
        print(f"\n--- Visualizing SAA Perturbation for {method} (Two Stages) ---")
        
        # --- Define helper functions as in the sampler ---
        def f(x, y_batch):
            y_b = y_batch
            if x.shape[0] != y_b.shape[0]:
                y_b = y_b.repeat_interleave(x.shape[0] // y_b.shape[0], dim=0)
            return criterion(saa_model(x), y_b)

        L_SMOOTHNESS, ALPHA, DELTA = 10.0, 1.0, 1.0
        M = (L_SMOOTHNESS**(2/(ALPHA+1))) / (((ALPHA+1)*DELTA)**((1-ALPHA)/(ALPHA+1)))
        d_dim = X_train_torch.shape[1]
        eta = 0.99 / (M * d_dim)

        def grad_f_y_eta(x):
            x.requires_grad_(True)
            x_orig = X_train_torch.detach()
            loss_val = f(x, y_train_torch)
            reg_val = torch.sum((x - x_orig)**2, dim=1) / (2 * eta)
            objective = loss_val + reg_val
            grads, = torch.autograd.grad(objective.sum(), x)
            x.requires_grad_(False)
        ...
        # --- Stage 1: Optimization Visualization ---
        opt_frames_dir = os.path.join(output_dir, f"saa_{method}_optimization_frames")
        os.makedirs(opt_frames_dir, exist_ok=True)
        opt_frames_dir_pdf = opt_frames_dir.replace(OUTPUT_DIR, OUTPUT_DIR_PDF)
        os.makedirs(opt_frames_dir_pdf, exist_ok=True)
        opt_gif_filenames = []

        L_g, mu_g, rho = 1/eta + M, max(0, 1/eta - M), math.sqrt(M * d_dim)
        w_initial = X_train_torch.clone().detach()

        w_final, w_history = accelerated_gradient_method_with_history(grad_f_y_eta, w_initial, L_g, mu_g, rho, max_iter=inner_steps)

        print("Generating Optimization GIF...")
        for step, w_current in enumerate(tqdm(w_history, desc="Optimization frames")):
            if step % (inner_steps // 20) == 0: # Plot ~20 frames
                frame_path = os.path.join(opt_frames_dir, f"frame_opt_{step:03d}.png")
                plot_frame_samples(saa_model, X_train, y_train, w_current, f"Optimization Process ({method})",
                                   frame_path, "SAA", f"Opt Step: {step+1}")
                opt_gif_filenames.append(frame_path)

        if imageio and opt_gif_filenames:
            gif_path = os.path.join(output_dir, f"saa_perturbation_{method}_optimization.gif")
            imageio.mimsave(gif_path, [imageio.imread(f) for f in opt_gif_filenames], duration=0.1)
            print(f"Optimization GIF saved to {gif_path}")

        # --- Stage 2: Rejection Sampling Visualization ---
        sampling_frames_dir = os.path.join(output_dir, f"saa_{method}_sampling_frames")
        os.makedirs(sampling_frames_dir, exist_ok=True)
        sampling_frames_dir_pdf = sampling_frames_dir.replace(OUTPUT_DIR, OUTPUT_DIR_PDF)
        os.makedirs(sampling_frames_dir_pdf, exist_ok=True)
        sampling_gif_filenames = []
        
        final_accepted_samples = torch.zeros((B, S, D), device=device)
        active_flags = torch.ones((B, S), dtype=torch.bool, device=device)

        w_final.requires_grad_(True)
        f_w = f(w_final, y_train_torch)
        grad_f_w, = torch.autograd.grad(f_w.sum(), w_final)
        w_final.requires_grad_(False)

        proposal_variance = eta / (1 - eta * M)
        proposal_std = math.sqrt(proposal_variance)
        s = grad_f_w + (1/eta)*(w_final - X_train_torch)
        proposal_mean = w_final - (eta / (1-eta*M))*s

        print("Generating Rejection Sampling GIF...")
        for trial in tqdm(range(40), desc="Sampling frames"):
            if not active_flags.any():
                break

            num_active = active_flags.sum()
            proposals = torch.randn((num_active, D), device=device) * proposal_std
            expanded_mean = proposal_mean.unsqueeze(1).expand(-1, S, -1)[active_flags]
            proposals += expanded_mean
            
            current_proposals = torch.zeros_like(final_accepted_samples)
            current_proposals[active_flags] = proposals

            # Simplified acceptance logic for visualization
            f_y_eta_X = torch.zeros_like(active_flags, dtype=torch.float32)
            h_1_y_w_X = torch.zeros_like(active_flags, dtype=torch.float32)

            if num_active > 0:
                f_X = f(proposals, y_train_torch.repeat_interleave(S)[active_flags.view(-1)])
                x_orig_expanded_active = x_original_expanded.view(B,S,D)[active_flags]
                norm_sq_X = torch.sum((proposals - x_orig_expanded_active)**2, dim=1)
                f_y_eta_X_active = f_X + norm_sq_X / (2 * eta)
                f_y_eta_X[active_flags] = f_y_eta_X_active
                
                w_expanded = w_final.unsqueeze(1).expand(-1, S, -1)[active_flags]
                grad_f_w_expanded = grad_f_w.unsqueeze(1).expand(-1, S, -1)[active_flags]
                f_w_expanded = f_w.unsqueeze(1).expand(-1, S)[active_flags]
                
                h_1_y_w_X_active = f_w_expanded + torch.sum(grad_f_w_expanded * (proposals - w_expanded), dim=1) - \
                                   (M/2) * torch.sum((proposals - w_expanded)**2, dim=1) + (1/(2*eta)) * norm_sq_X - ((1 - ALPHA) * DELTA) / 2
                h_1_y_w_X[active_flags] = h_1_y_w_X_active
            ...
            # --- Objective History Calculation ---
            with torch.no_grad():
                loss_values_obj = criterion(saa_model(final_accepted_samples.view(-1, D)), y_repeated)
                dist_sq_obj = torch.sum((final_accepted_samples.view(-1, D) - x_original_expanded)**2, dim=1)
                objective_per_sample = loss_values_obj - LAMBDA_PARAM * dist_sq_obj
                objective_history.append(objective_per_sample.mean().item())
            
            active_flags[newly_accepted_mask] = False

            frame_path = os.path.join(sampling_frames_dir, f"frame_sampling_{trial:03d}.png")
            plot_frame_samples(saa_model, X_train, y_train, final_accepted_samples.view(-1, D), 
                               f"Rejection Sampling ({method})",
                               frame_path, "SAA", f"Trial: {trial+1}")
            sampling_gif_filenames.append(frame_path)

        if imageio and sampling_gif_filenames:
            gif_path = os.path.join(output_dir, f"saa_perturbation_{method}_sampling.gif")
            imageio.mimsave(gif_path, [imageio.imread(f) for f in sampling_gif_filenames], duration=0.2)
            print(f"Sampling GIF saved to {gif_path}")
            
        return objective_history

    if method == "SVGD":
        particles = X_train_torch.unsqueeze(1).repeat(1, S, 1) + torch.randn(B, S, D, device=device) *0.5 #* np.sqrt(EPSILON / 2)
        hist_grad = torch.zeros_like(particles)
    else:
        particles = x_original_expanded.clone()
        if method == 'WFR':
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
        ...
        if method == "SVGD":
            ...
            total_grad = (grads.view(-1,D) + grad_log_px).view(B, S, D)/LAMBDA_PARAM/EPSILON #* step / inner_steps
            K, grad_K_x = rbf_kernel_batched_torch(particles)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "SVGD", "MMD", "Dual", "WGF", "WRM", "WFR", "ProximalRejection"]
    loss_histories = {name: [] for name in ["SVGD", "Dual", "WGF", "WFR", "MMD", "ProximalRejection"]}
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["SVGD", "WRM", "Dual", "ProximalRejection"] #"MMD", "WFR", "WGF", 
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                    ...
                    model.train()
                    loss = criterion(model(X_perturbed), y_repeated)
                elif method == "ProximalRejection":
                    model.eval()
                    X_perturbed = proximal_rejection_sampler(X_batch, y_batch, model, epoch, inner_steps=inner_steps)
                    y_repeated = y_batch.repeat_interleave(NUM_SAMPLES_PER_POINT)
                    model.train()
                    loss = criterion(model(X_perturbed), y_repeated)
            ...
            if method in ["SVGD", "WGF", "WRM", "WFR", "MMD", "ProximalRejection"]:
                model.eval()
                epoch_weights = None 
                if method == "WGF":
                    ...
                    epoch_X_p = proximal_rejection_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, epoch, inner_steps=inner_steps)
        ...
        if imageio and method in ["WGF", "WRM", "WFR", "MMD", "SVGD", "ProximalRejection"]:
