...
def rbf_kernel_batched_torch(particles):
    ...
    h_squared =  5 * median_sq_dist / (torch.log(torch.tensor(S, device=device)) + 1e-8)
...
def svgd_sampler(
    ...
    particles += torch.randn_like(particles) * np.sqrt(0.1)
...
if __name__ == '__main__':
    ...
    all_methods = ["SVGD", "WGF", "WFR", "WRM", "MMD"]
