...
MMD_NOISE_STD = 1e-2 # Noise for MMD Langevin Dynamics
...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=300, num_samples_per_point=5):
    """
...
def plot_perturbation_process_mmd_with_forces(saa_model, X_train, y_train, output_dir, lr=0.01, inner_steps=300):
    """
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating MMD frames"):
        ...
        _, grad_K_clone = rbf_kernel_and_grad(x_clone_detached, x_clone_detached)
        mmd_term1 = torch.mean(grad_K_clone, dim=1)
        
        _, grad_K_orig = rbf_kernel_and_grad(x_clone_detached, X_train_torch)
