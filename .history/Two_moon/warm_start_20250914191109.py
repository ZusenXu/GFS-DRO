...
def wgf_sampler_warm_start(x_original_batch, y_original_batch, model, initial_particles, lr=inner_lr, inner_steps=inner_steps_warm_start):
    """ WGF sampler that starts from initial_particles (热启动 WGF 采样器). """
...
def wfr_sampler_warm_start(x_original_batch, y_original_batch, model, initial_particles, initial_weights, lr=inner_lr, inner_steps=inner_steps_warm_start):
    """ WFR sampler that starts from initial_particles and initial_weights (热启动 WFR 采样器). """
...
def plot_frame_samples(model, X, y, X_perturbed, title, save_path, method, epoch, weights=None):
    ...
    ax.scatter(X[y==1, 0], X[y==1, 1], c='darkorange', marker='o', edgecolors='k', label='正样本数据', alpha=0.5, s=20, linewidths=0.5)
    ax.scatter(X[y==0, 0], X[y==0, 1], c='dodgerblue', marker='o', edgecolors='k', label='负样本数据', alpha=0.5, s=20, linewidths=0.5)

    if X_perturbed is not None:
        X_p_np = X_perturbed.cpu().numpy()
        
        # If weights are not provided, create them assuming uniform distribution
        if weights is None:
            num_repeats = X_p_np.shape[0] // X.shape[0]
            weights = torch.ones((X.shape[0], num_repeats), device=X_perturbed.device) / num_repeats

        # Defensive check to prevent crash from shape mismatch
        ...
        # Robustly create the repeated labels array based on the final consistent shapes
    ...
    legend_elements = [
        Line2D([0], [0], color='black', linestyle='--', lw=2, label=f'决策边界 ({method})'),
        Line2D([0], [0], linestyle='None', marker='o', color='darkorange', label='正样本', markersize=5, markeredgecolor='k', markeredgewidth=0.5),
        Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', label='负样本', markersize=5, markeredgecolor='k', markeredgewidth=0.5),
        Line2D([0], [0], linestyle='None', marker='o', color='darkorange', alpha=0.6, label='扰动后的正样本', markersize=5, markeredgecolor="gray", markeredgewidth=0.2),
        Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', alpha=0.6, label='扰动后的负样本', markersize=5, markeredgecolor="gray", markeredgewidth=0.2)
    ]

    ax.set_xlabel('特征 1', fontsize=9)
    ax.set_ylabel('特征 2', fontsize=9)
...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path_png):
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
    ax.scatter(X[y==1, 0], X[y==1, 1], c='darkorange', marker='o', edgecolors='k', label='正样本数据', alpha=0.5, linewidths=0.5, s=20)
    ax.scatter(X[y==0, 0], X[y==0, 1], c='dodgerblue', marker='o', edgecolors='k', label='负样本数据', alpha=0.5, linewidths=0.5, s=20)
    ...
    legend_elements = [
        Line2D([0], [0], linestyle='None', marker='o', color='darkorange', label='正样本', markersize=4, markeredgecolor='k', markeredgewidth=0.5),
        Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', label='负样本', markersize=4, markeredgecolor='k', markeredgewidth=0.5),
        Line2D([0], [0], color="black", lw=1.5, linestyle="-", label="理想边界")
    ]
    ...
    ax.set_xlabel('特征 1'); ax.set_ylabel('特征 2'); ax.set_xlim(xx.min(), xx.max()); ax.set_ylim(yy.min(), yy.max())
...
if __name__ == '__main__':
    ...
    # --- DRO Models Training ---
    # MODIFIED: Renamed WGF/WFR to indicate warm start for clarity
    ...
    for method_name, method_key in dro_methods_to_train.items():
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method_name} Training"):
            ...
            plot_frame_samples(model, X_train, y_train, epoch_X_p, f"最差情况下的样本分布 ({method_name})", samples_frame_path, method_name, epoch, weights=epoch_weights)
            gif_filenames.append(samples_frame_path)

        trained_models[method_name] = model

        if imageio and gif_filenames:
            print(f"正在为 {method_name} 创建 GIF 动画...")
    ...
    visualize_all_boundaries(trained_models, X_train, y_train, "所有模型的决策边界对比", f"{OUTPUT_DIR}/final_boundary_comparison.png")
    print(f"最终边界对比图已保存。")

    print("\n--- 正在评估最终模型的鲁棒性 ---")
    print("-" * 75)
    print(f"攻击强度 (Epsilon): {ATTACK_EPSILON}")
    print("-" * 75)
    print(f"| {'模型':<15} | {'原始准确率':<15} | {'FGSM 攻击后准确率':<20} | {'PGD 攻击后准确率':<20} |")
    print("-" * 75)
    for name, model in trained_models.items():
        ...
        print(f"| {name:<15} | {acc_clean:^15.2%} | {acc_fgsm:^22.2%} | {acc_pgd:^21.2%} |")
    print("-" * 75)

    print(f"\n实验完成。所有结果已保存在 '{OUTPUT_DIR}' 和 '{OUTPUT_DIR_PDF}' 文件夹中。")
