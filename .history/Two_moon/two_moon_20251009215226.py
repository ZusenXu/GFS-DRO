...
def svg_sampler(
    ...
    for _ in range(inner_steps):
        ...
        with torch.no_grad():
            hist_grad = adagrad_hist_decay * hist_grad + (1 - adagrad_hist_decay) * (svg_grad**2)
...
def rgo_sampler(x_original_batch, y_original_batch, model, epoch, inner_lr=inner_lr, inner_steps=inner_steps):
    ...
    for _ in range(steps):
        ...
        grad_total = -grads + 2 * LAMBDA_PARAM * (x_pert_batch - x_original_batch)
...
def plot_frame(model, X, y, X_perturbed, save_path, method, epoch):
...
def plot_frame_samples(model, X, y, X_perturbed, save_path, method, epoch, weights=None):
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
...
def plot_frame_samples_with_forces(model, X, y, X_perturbed, save_path, method, epoch, force1, force2, weights=None):
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
    ...
    force_labels = {
        ...
        'SVG': (r'driving force', r'repulsive force')
    }
...
def plot_perturbation_process_kernel_force(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=200, adagrad_hist_decay=0.9):
    ...
    if method == "MMD":
        ...
        for step in tqdm(range(inner_steps), desc=f"Generating {method} force frames"):
            ...
            if step % 5 == 0:
                 frame_path = os.path.join(frames_dir, f"frame_{step:03d}.png")
                 plot_frame_samples_with_forces(saa_model, X_train, y_train, particles,  
                                                frame_path, method, f"Step: {step+1}", 0.2 * force1_loss_grad, 0.2 * force2_kernel_grad)
                 gif_filenames.append(frame_path)

    elif method == "SVG":
        particles = X_train_torch.unsqueeze(1).repeat(1, S, 1) + torch.randn(B, S, D, device=device) * 0.1
        hist_grad = torch.zeros_like(particles)
        for step in tqdm(range(inner_steps), desc=f"Generating {method} force frames"):
            with torch.no_grad():
            ...
            svg_grad = force1_driving + force2_repulsive
            ...
            if step % 4 == 0:
                 frame_path = os.path.join(frames_dir, f"frame_{step:03d}.png")
                 plot_frame_samples_with_forces(saa_model, X_train, y_train, particles.view(-1, D), 
                                                frame_path, method, f"Step: {step+1}",  LAMBDA_PARAM*EPSILON * force1_driving.view(-1,D),  LAMBDA_PARAM*EPSILON * force2_repulsive.view(-1,D))
                 gif_filenames.append(frame_path)
...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.001, inner_steps=500, adagrad_hist_decay=0.9, svg_noise_std=0.1):
    ...
    if method == 'RGO':
        opt_steps = inner_steps
        sampling_viz_steps = 40  
        frame_counter = 0

        # --- Phase 1: Optimization ---
        print(f"\n--- Visualizing RGO Phase 1: Optimization ---")
        x_pert_batch = X_train_torch.clone().detach()
        for step in tqdm(range(opt_steps), desc=f"Generating RGO Optimization frames"):
            x_pert_batch.requires_grad_(True)
            loss_values = criterion(saa_model(x_pert_batch), y_train_torch)
            
            with torch.no_grad():
                dist_sq = torch.sum((x_pert_batch - X_train_torch)**2, dim=1)
                objective = (loss_values - LAMBDA_PARAM * dist_sq).mean().item()
                objective_history.append(objective)

            grads, = torch.autograd.grad(loss_values.sum(), x_pert_batch)
            x_pert_batch = x_pert_batch.detach()
            grad_total = -grads + 2 * LAMBDA_PARAM * (x_pert_batch - X_train_torch)
            x_pert_batch -= lr * grad_total

            if step % 10 == 0:
                frame_path = os.path.join(frames_dir, f"frame_{frame_counter:04d}.png")
                plot_X_p = x_pert_batch.unsqueeze(1).repeat(1, S, 1).view(-1, D)
                plot_frame_samples(saa_model, X_train, y_train, plot_X_p, 
                                   frame_path, "SAA", f"Opt Step: {step+1}", weights=None)
                gif_filenames.append(frame_path)
                frame_counter += 1
        ...
        # --- Phase 2: Sampling ---
        print(f"\n--- Visualizing RGO Phase 2: Sampling ---")
        var_rgo, std_rgo = EPSILON, math.sqrt(EPSILON)
        
        with torch.no_grad():
            f_model_loss_opt_star = criterion(saa_model(x_opt_star_batch), y_train_torch)
            norm_sq_opt_star = torch.sum((x_opt_star_batch - X_train_torch) ** 2, dim=1)
            f_L_xi_opt_star = (-f_model_loss_opt_star / (LAMBDA_PARAM * EPSILON)) + (norm_sq_opt_star / EPSILON)

        x_opt_star_3d = x_opt_star_batch.unsqueeze(1)
        x_original_3d = X_train_torch.unsqueeze(1)
        f_L_xi_opt_star_3d = f_L_xi_opt_star.unsqueeze(1)
        
        final_accepted_perturbations = torch.zeros((B, S, D), device=device)
        active_flags = torch.ones((B, S), dtype=torch.bool, device=device)
        y_repeated_sampling = y_train_torch.repeat_interleave(S)

        for step in tqdm(range(sampling_viz_steps), desc=f"Generating RGO Sampling frames"):
            if not active_flags.any(): break
            ...
            with torch.no_grad():
                x_candidates_flat = x_candidates.view(-1, D)
                f_model_loss_candidates = criterion(saa_model(x_candidates_flat), y_repeated_sampling).view(B, S)
                norm_sq_candidates = torch.sum((x_candidates - x_original_3d) ** 2, dim=2)
                f_L_xi_candidates = (-f_model_loss_candidates / (LAMBDA_PARAM * EPSILON)) + (norm_sq_candidates / EPSILON)
                diff_cand_opt_norm_sq = torch.sum(pert_proposals**2, dim=2)
                exponent_term3 = diff_cand_opt_norm_sq / (2 * var_rgo)
            ...
            with torch.no_grad():
                current_samples_flat = (x_opt_star_3d + final_accepted_perturbations).view(-1, D)
                loss_values = criterion(saa_model(current_samples_flat), y_repeated_sampling)
                dist_sq = torch.sum((current_samples_flat - x_original_expanded)**2, dim=1)
                objective = (loss_values - LAMBDA_PARAM * dist_sq).mean().item()
                objective_history.append(objective)

            if step % 2 == 0:
                frame_path = os.path.join(frames_dir, f"frame_{frame_counter:04d}.png")
                plot_X_p = (x_opt_star_3d + final_accepted_perturbations).view(-1, D)
                plot_frame_samples(saa_model, X_train, y_train, plot_X_p, 
                                   frame_path, "SAA", f"Sample Step: {step+1}", weights=None)
                gif_filenames.append(frame_path)
                frame_counter += 1
    
    else:
        weights = None
        if method.startswith("SVG"):
            particles = X_train_torch.unsqueeze(1).repeat(1, S, 1) + torch.randn(B, S, D, device=device) * svg_noise_std
            hist_grad = torch.zeros_like(particles)
        else:
            particles = x_original_expanded.clone()
            if method == 'WFR':
        ...
        for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
            ...
            if method.startswith("SVG"):
                ...
                K, grad_K_x = rbf_kernel_batched_torch(particles)
                svg_grad = (torch.bmm(K, total_grad) + torch.sum(grad_K_x, dim=2)) / S
                with torch.no_grad():
                    hist_grad = adagrad_hist_decay * hist_grad + (1 - adagrad_hist_decay) * (svg_grad**2)
                    particles += lr * svg_grad / (1e-6 + torch.sqrt(hist_grad)) 
            elif method == 'MMD':
                _, grad_K_clone = imq_kernel_and_grad(particles, particles)
                _, grad_K_orig = imq_kernel_and_grad(particles, X_train_torch)
            ...
            if step % 10 == 0:
                frame_path = os.path.join(frames_dir, f"frame_{step:03d}.png")
                plot_frame_samples(saa_model, X_train, y_train, particles.view(-1,D), 
...
def plot_objective_comparison(histories: Dict[str, list], save_path_png: str):
    plt.figure(figsize=(4.0, 2.613), dpi=300, tight_layout=True)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "SVG", "MMD", "Dual", "WGF", "WRM", "WFR", "RGO"]
    loss_histories = {name: [] for name in ["SVG", "Dual", "WGF", "WFR", "MMD", "RGO"]}
    ...
    all_methods = ["SVG", "RGO", "WRM", "WGF", "WFR"]
    comparison_histories = {}
    for method in all_methods:
        if method == "SVG":
            plot_perturbation_process_kernel_force(saa_model, X_train, y_train, method, OUTPUT_DIR, lr=0.01, inner_steps=300)
            # Run for SVG with deviation 0.1
            history_01 = plot_perturbation_process(saa_model, X_train, y_train, "SVG_0.1", OUTPUT_DIR, lr=0.01, inner_steps=300, svg_noise_std=0.1)
            comparison_histories["SVG_0.1"] = history_01
            # Run for SVG with deviation 0.2
            history_02 = plot_perturbation_process(saa_model, X_train, y_train, "SVG_0.2", OUTPUT_DIR, lr=0.01, inner_steps=300, svg_noise_std=0.2)
            comparison_histories["SVG_0.2"] = history_02
            
        else:
            history = plot_perturbation_process(saa_model, X_train, y_train, method, OUTPUT_DIR, lr=0.01, inner_steps=300)
            if method in ["WGF", "WFR", "WRM", "RGO"]:
    ...
    dro_methods_to_train = ["SVG", "RGO", "WRM", "Dual", "WFR", "WGF"]
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                    ...
                    model.eval()
                    X_perturbed = svg_sampler(X_batch, y_batch, model)
                    y_repeated = y_batch.repeat_interleave(NUM_SAMPLES_PER_POINT)
                    model.train()
                    loss = criterion(model(X_perturbed), y_repeated)
                elif method == "RGO":
            ...
            if method in ["SVG", "WGF", "WRM", "WFR", "MMD", "RGO"]:
                model.eval()
                epoch_weights = None 
                if method == "WGF":
                    ...
                    epoch_X_p = svg_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model)
                elif method == "RGO":
                    epoch_X_p = rgo_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, epoch)

                frame_path = os.path.join(method_frames_dir, f"{method}_frame_{epoch:03d}.png")
                plot_frame(model, X_train, y_train, epoch_X_p, frame_path, method, epoch)
                
                samples_frame_path = os.path.join(method_frames_dir, f"{method}_frame_{epoch:03d}_samples.png")
                plot_frame_samples(model, X_train, y_train, epoch_X_p, samples_frame_path, method, epoch, weights=epoch_weights)
        ...
        if imageio and method in ["WGF", "WRM", "WFR", "MMD", "SVG", "RGO"]:
