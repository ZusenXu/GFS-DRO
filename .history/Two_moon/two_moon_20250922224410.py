...
def svgd_sampler(
    ...
    """
    A pure PyTorch, parallelized version of the SVGD sampler.
    All operations are performed on the specified device (e.g., GPU)
    without transferring data to the CPU.
    """
    device = x_original_batch.device
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "Dual":
                    ...
                    model.eval()
                    X_perturbed = svgd_sampler(X_batch, y_batch, model, epoch)
                    y_repeated = y_batch.repeat_interleave(NUM_SAMPLES_PER_POINT)
                    model.train()
                    loss = criterion(model(X_perturbed), y_repeated)
            ...
            if method in ["SVGD", "WGF", "WRM", "WFR", "MMD"]:
                ...
                if method == "WGF":
                    ...
                    epoch_X_p = svgd_sampler(train_dataset.tensors[0].to(DEVICE), train_dataset.tensors[1].to(DEVICE), model, epoch)
