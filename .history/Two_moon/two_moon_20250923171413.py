...
def plot_perturbation_process(saa_model, X_train, y_train, method, output_dir, lr=0.01, inner_steps=200, adagrad_hist_decay=0.9):
    """Generates GIFs WITHOUT force visualizations and returns objective history for ALL methods."""
    ...
    # Initialize particles and method-specific variables
    weights = None
    if method == "SVGD":
        particles = X_train_torch.unsqueeze(1).repeat(1, S, 1) + torch.randn(B, S, D, device=device) * np.sqrt(EPSILON / 2)
        hist_grad = torch.zeros_like(particles)
    ...
    for step in tqdm(range(inner_steps), desc=f"Generating {method} frames"):
        ...
        if method == "SVGD":
            ...
            total_grad = (grads.view(-1,D) + grad_log_px).view(B, S, D)
            K, grad_K_x = rbf_kernel_batched_torch(particles)
            ...
            if method == 'WRM':
                ...
                if method == 'WFR':
                    ...
                    # --- Added Resampling Logic for WFR Visualization ---
                    with torch.no_grad():
                        low_weight_mask = weights < 1e-4
                        rows_with_low_weights = torch.any(low_weight_mask, dim=1)
                        if torch.any(rows_with_low_weights):
...
if __name__ == '__main__':
    ...
    all_methods = ["WGF", "WFR", "WRM", "SVGD", "MMD"]
    comparison_histories = {}
    for method in all_methods:
        history = plot_perturbation_process(saa_model, X_train, y_train, method, OUTPUT_DIR, lr=inner_lr, inner_steps=300)
        if method in ["WGF", "WFR", "WRM", "SVGD"]:
            comparison_histories[method] = history

    # 2. Plot the collected objective histories
    plot_objective_comparison(comparison_histories, os.path.join(OUTPUT_DIR, "saa_perturb_objective_comparison.png"))

    # 3. Generate WITH-FORCE GIFs (optional, for debugging/analysis)
    # for method in all_methods:
