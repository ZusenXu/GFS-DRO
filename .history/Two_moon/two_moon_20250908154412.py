...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=50):
    ...
    std_dev = torch.sqrt(torch.tensor(2 * lr * LAMBDA_PARAM * EPSILON, device=device))
    weight_exponent = 1 - LAMBDA_PARAM * EPSILON * lr
    ...
    for _ in range(inner_steps):
        ...
        with torch.no_grad():
            ...
            weights = weights ** weight_exponent * torch.exp(-new_loss * lr)
...
if __name__ == '__main__':
    ...
    model_names = ["SAA", "RGO", "Dual", "MultiLD", "WDRO", "SVGD", "WFR"]#"LD", 
    loss_histories = {name: [] for name in ["RGO", "Dual", "MultiLD", "SVGD", "WFR"]}  # SAA does not use DRO loss "LD", "WDRO", 
    ...
    # --- DRO Models Training ---
    dro_methods_to_train = ["WDRO", "WFR", "MultiLD", "RGO", "Dual", "SVGD"] #"LD", 
    ...
    for method in dro_methods_to_train:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            if epoch % LOG_EPOCH_INTERVAL == 0 & method in loss_histories:
