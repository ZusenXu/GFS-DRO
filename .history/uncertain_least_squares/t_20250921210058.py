...
# WGF, WFR, RGO, MMD-DRO Specific Parameters
...
def solve_wrm(xi_train_samples):
    ...
    for epoch in range(N_EPOCHS):
        ...
        for _ in range(K_INNER_STEPS):
            ...
            z_k += INNER_STEP_SIZE * grad
            z_k = np.clip(z_k, -1, 1) # THIS LINE IS NOW RESTORED
...
def run_experiment():
    ...
    results = {'ERM': [], 'RGO': [], 'MMD-DRO':[], 'WGF': [], 'WFR': [], 'WRM': [], 'Dual': []}
    models = {
        'ERM': theta_erm, 'RGO': theta_rgo, 'MMD-DRO': theta_mmd_dro, 'WGF': theta_wgf, 
...
def plot_results(delta_values, results):
    ...
    styles = {
        ...
        'MMD-DRO': {'color': 'cyan', 'linestyle': (0, (3, 1, 1, 1, 1, 1))}, # cyan, dashdotdotted
    ...
    plt.tight_layout()
    plt.savefig('uncertain_least_squares_results_complete.png', dpi=300)
    plt.show()
