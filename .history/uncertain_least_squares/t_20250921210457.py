...
def solve_mmd_dro(xi_train_samples):
    ...
    for epoch in range(N_EPOCHS):
        ...
        for _ in range(current_inner_steps):
            ...
            noise = np.random.randn(*particles_flat.shape) * 0.0001
...
def run_experiment():
    ...
    print("Solvinh MMD-DRO model...")
    theta_mmd_dro = solve_mmd_dro(xi_train)
    print("All models solved.")

    delta_values = np.linspace(DELTA_MIN, DELTA_MAX, DELTA_STEPS)
    results = {'ERM': [], 'WGF': [], 'WFR': [], 'WRM': [], 'Dual': [], 'MMD-DRO': []}
    models = {
        'ERM': theta_erm,  'WGF': theta_wgf, 
        'WFR': theta_wfr, 'WRM': theta_wrm, 'Dual': theta_dual, 'MMD-DRO': theta_mmd_dro
    }
...
def plot_results(delta_values, results):
    ...
    styles = {
        ...
        'MMD-DRO': {'color': '#8c564b', 'linestyle': (0, (1, 1))} # brown, dotted
    }
