...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path):
    """MODIFIED to use a hardcoded, high-contrast color and style map."""
    fig, ax = plt.subplots(figsize=(4.2, 2.613), dpi=300, tight_layout=True)
    
    # Define a clear color and style for each model
    style_map = {
        "MultiLD": {"color": "darkgreen", "linestyle": "--"},
        "Sinha": {"color": "darkorange", "linestyle": "--"}
    }
    # Default style for any models not in the map
    default_style = {"color": "black", "linestyle": "-."}

    # Plot data points
    ax.scatter(X[y==1, 0], X[y==1, 1], c='red', marker='o', edgecolors='k', label='Positive Data', alpha=0.5)
    ...
    # Plot each model's boundary using the specified style
    for name, model in models.items():
        model.eval()
        with torch.no_grad():
            Z = model(grid).argmax(dim=1).cpu().numpy().reshape(xx.shape)
        
        # Get the style from the map or use the default
        style = style_map.get(name, default_style)
        ax.contour(xx, yy, Z, levels=[0.5], colors=[style["color"]], linestyles=style["linestyle"], linewidths=2)
    ...
    # Add legend entries for each model boundary
    for name in models.keys():
        style = style_map.get(name, default_style)
        legend_elements.append(
            Line2D([0], [0], color=style["color"], lw=2, linestyle=style["linestyle"], label=name)
        )
