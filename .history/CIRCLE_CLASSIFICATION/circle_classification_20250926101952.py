...
def wgf_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=40):
...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=40):
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=40):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path):
    ...
    colors = ['seagreen', 'purple', "darkred"]
    linestyles = ['--', '--', '--']
    ...
    for method in model_names:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                if method == "WGF":
                    model.eval()
                    X_perturbed = wgf_sampler(X_batch, y_batch, model)
