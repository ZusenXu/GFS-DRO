...
def wgf_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=30):
...
def wfr_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=30):
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=1e-2, inner_steps=30):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
