...
def classification_SNVD20(n_samples, seed=42, is_train=False):
    ...
    if is_train:
        ...
        is_second_quadrant = (X_intermediate[:, 0] < 0) & (X_intermediate[:, 1] > 0) & (-0.1*X_intermediate[:, 1] > X_intermediate[:, 0])
        is_third_quadrant = (X_intermediate[:, 0] < 0) & (X_intermediate[:, 1] < 0)
        is_fourth_quadrant = (X_intermediate[:, 0] > 0) & (X_intermediate[:, 1] < 0)
        neg_q1_indices = np.where(is_second_quadrant)[0] #is_negative & is_up & 
...
def plot_frame_samples(model, X, y, X_perturbed, title, save_path, method, epoch):
    ...
    legend_elements = [
        ...
        Line2D([0], [0], linestyle='None', marker='x', color='darkorange', label='Perturbed Positive', markersize=5, markeredgecolor='k'),
        Line2D([0], [0], linestyle='None', marker='x', color='dodgerblue', label='Perturbed Negative', markersize=5, markeredgecolor='k')
    ]
...
if __name__ == '__main__':
    ...
    visualize_all_boundaries(trained_models, X_train, y_train, "Decision Boundaries", f"{OUTPUT_DIR}/circle_boundary_comparison.png")
    print(f"Final boundary comparison plot saved to {OUTPUT_DIR}/circle_boundary_comparison.png")
