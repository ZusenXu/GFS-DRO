...
N_SAMPLES_TEST = 1000
INPUT_DIM = 2
HIDDEN_DIM = 4  # UPDATED: Hidden dimension changed to 4
OUTPUT_DIM = 2
MAX_EPOCHS = 10
BATCH_SIZE = 64
LR = 5e-2
...
# --- 2. Model and Data ---
# UPDATED: Model changed to a two-layer network with one hidden layer
...
# MODIFIED: Dataset generation function as per user request
def classification_SNVD20(n_samples, seed=42, is_train=False):
    ...
    """
    np.random.seed(seed)
    # Generate more points to ensure enough samples after filtering and potential deletion
    # Increased multiplier to 10 to be safe
    X = np.random.randn(n_samples * 10, 2)
    norms = np.linalg.norm(X, axis=1)
    y = np.sign(norms - np.sqrt(2)) # y is {-1, 1}

    # Original filtering based on the gap
    ...
    if is_train:
        # Identify negative samples (label is -1) in the first quadrant
        is_negative = (y_intermediate == -1)
        is_first_quadrant = (X_intermediate[:, 0] > 0) & (X_intermediate[:, 1] > 0)
        
        # Get the indices of these specific points
        neg_q1_indices = np.where(is_negative & is_first_quadrant)[0]
        
        # Determine the number of samples to remove (4/5)
        num_to_remove = int(len(neg_q1_indices) * 9 / 10)
        
        if num_to_remove > 0:
            # Randomly choose the indices of the points to remove
            indices_to_remove = np.random.choice(neg_q1_indices, size=num_to_remove, replace=False)
            
            # Create a boolean mask to keep all other points
            deletion_mask = np.ones(len(X_intermediate), dtype=bool)
            deletion_mask[indices_to_remove] = False
            
            # Apply the mask to get the final filtered dataset
...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path):
    ...
    for i, (name, model) in enumerate(models.items()):
        ...
        ax.contour(xx, yy, Z, levels=[0.5], colors=[colors[i]], linewidths=2)
    legend_elements = [Line2D([0], [0], linestyle='None', marker='o', color='darkorange', label='Positive Data', markersize=4, markeredgecolor='k'),
                       Line2D([0], [0], linestyle='None', marker='o', color='dodgerblue', label='Negative Data', markersize=4, markeredgecolor='k'),]
    legend_elements.extend([Line2D([0], [0], color=colors[i], lw=2, linestyle="-", label=name) for i, name in enumerate(models.keys())])
...
if __name__ == '__main__':
    # --- Data ---
    # Using a common, available style
    plt.style.use('seaborn-v0_8-paper') 
    
    # MODIFIED: Using the new dataset function with the is_train flag
    ...
    # UPDATED: Kept only Sinha and MultiLD
    model_names = ["MultiLD", "Sinha"]
    ...
    for method in model_names:
        ...
        for epoch in tqdm(range(MAX_EPOCHS), desc=f"{method} Training"):
            ...
            for X_batch, y_batch in train_loader:
                ...
                # UPDATED: Simplified training loop for only the two methods
                if method == "MultiLD":
                    model.eval()
