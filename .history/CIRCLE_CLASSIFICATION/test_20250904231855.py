...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=5e-2, inner_steps=10):
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=5e-2, inner_steps=10):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
