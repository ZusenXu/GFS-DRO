...
# NEW: Dataset generation function as per user request
def classification_SNVD20(n_samples, seed=42):
    """
    Generates synthetic data as described in Section 5.1 of the paper.
    """
    np.random.seed(seed)
    # Generate more points to ensure enough samples after filtering
...
def mul_ld_sampler(x_original_batch, y_original_batch, model, lr=1e-1, inner_steps=10):
...
def sinha_sampler(x_original_batch, y_original_batch, model, lr=1e-1, inner_steps=10):
    x_clone = x_original_batch.clone().detach().requires_grad_(True)
    for _ in range(inner_steps):
...
def visualize_all_boundaries(models: Dict[str, nn.Module], X, y, title, save_path):
    ...
    xx, yy = np.meshgrid(np.arange(x_min, x_max, 0.02), np.arange(y_min, y_max, 0.02))
    grid = torch.tensor(np.c_[xx.ravel(), yy.ravel()], dtype=torch.float32).to(DEVICE)
    colors = plt.cm.viridis(np.linspace(0, 0.5, len(models)))
    linestyles = ['--', ':', '-.', '--']
...
if __name__ == '__main__':
    ...
    # UPDATED: Using the new dataset function
    X_train, y_train = classification_SNVD20(n_samples=N_SAMPLES_TRAIN, seed=SEED)
    X_test, y_test = classification_SNVD20(n_samples=N_SAMPLES_TEST, seed=SEED+1)
